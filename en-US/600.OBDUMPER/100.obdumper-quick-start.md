...
```
  ...
  <main id="notice" type='explain'>
    <h4>Note</h4>
    <p>The <code>--sys-user</code> option is used to connect to a user with required privileges in the <code>sys</code> tenant. If the <code>--sys-user</code> option is not specified during the export, the default value <code>--sys-user root</code> is used. In OceanBase Database V4.0.0 and later, you do not need to specify this option. For more information, see <a href="200.obdumper-command-line-options.md">OBDUMPER command-line options</a>. </p>
  </main>
