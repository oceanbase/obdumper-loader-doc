...
* --sequence *'**sequence_name [, sequence_name...]**'* | --sequence '**\***'
  ...
  * By default or when `preserve` is specified, the start value is set to the current sequence progress during export.

  * When `restart` is specified, the start value is set to the start value when the table was created during export.

  The start value of generated columns when exporting table definitions can be controlled using the --sequence-policy option. By default, the start value is set to the current sequence progress when `preserve` is specified, and to the start value when the table was created when `restart` is specified.
...
* --flashback-timestamp '***timestamp_string***'
  ...
  Used to export the flashback point data. This option can be used only with any one of the data format options and cannot be used with the `--query-sql` option. This option applies only to the Oracle compatible mode of OceanBase Database.
...
* Table-level blocklist and allowlist filtering
...
* Column-level blocklist and allowlist filtering
...
| --skip-header             | No     | Specifies whether to ignore the file header (first row of field names) of the CSV/POS file. The value is of the BOOLEAN type.                                   |          |          |
