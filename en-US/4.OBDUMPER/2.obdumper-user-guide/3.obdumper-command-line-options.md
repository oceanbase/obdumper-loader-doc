...
* --table ' ***table_name [,table_name...]*** ' | --table ' **\*** '
  ...
  Exports table definitions or table data. When this option is used in combination with `--ddl`, only table definitions are exported. When this option is used in combination with any data format option, only table data is exported. To specify multiple tables, separate the table names with commas (,). Table names in Oracle mode are case-sensitive by default, while table names in MySQL mode are case-insensitive by default. If case sensitivity is required, you can enable the parameter `lower_case_table_names`, and put the table name in square brackets. Example: When the table name in MySQL mode is case-sensitive, `--table 'test'` and `--table 'TEST'` represent the test table, `--table '[test]'` represents the test table, and `--table '[TEST]'` represents the TEST table. If the table name is specified as an asterisk (*), it means to export all table definitions or table data.
<br><br>
