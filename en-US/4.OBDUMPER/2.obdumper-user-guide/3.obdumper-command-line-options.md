...
* --exclude-column-names **'** ***column_name [, column_name...]*** **'** 
  ...
  Excludes the specified columns from the export of table data. Fuzzy match on column names is not supported. To specify multiple tables, separate the table names with commas (,). Table names of tenants in Oracle mode are uppercase by default, while table names of tenants in MySQL mode are lowercase by default. Example: In OceanBase Oracle mode, `--table 'test'` and `--table 'TEST'` represent the TEST table. In OceanBase MySQL mode, `--table 'test'` and `--table 'TEST'` represent the test table. If case sensitivity is required, you can enclose the table name in square brackets ([ ]). Example: `--table '[test]'` represents the test table, and `--table '[TEST]'` represents the TEST table. If the table name is specified as an asterisk (*), it means to export all table definitions or table data.
  > **Notice**  
  > In the exported data file, there is no data in the excluded specified column.
