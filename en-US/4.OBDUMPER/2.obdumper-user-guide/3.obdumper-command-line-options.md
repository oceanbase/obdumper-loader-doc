...
* --pl-delimiter **'** ***delimiter_string*** **'**
  ...
  Inserts a delimiter string before and after the PL/SQL object creation statement when exporting PL/SQL definitions. This option can be used only in combination with the `--ddl` option. Default value: $$.
<br><br>
* --snapshot
