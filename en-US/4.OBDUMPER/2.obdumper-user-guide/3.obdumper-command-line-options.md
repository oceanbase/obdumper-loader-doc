...
* --all
  ...
  Exports all database object definitions and table data. When this option is used in combination with `--ddl`, all database object definitions are exported. When this option is used in combination with `--csv`, `--sql`, `--cut`, or `--pos`, data in all tables is exported in the specified format. To export all database object definitions and table data, you can specify `--all` and `--ddl` along with a data format option. 
  > **Notice**  
