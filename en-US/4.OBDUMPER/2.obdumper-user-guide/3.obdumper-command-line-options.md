...
| --time-value-format       | No       | The export format of the TIME data type in MySQL mode.  |          |          |
| --datetime-value-format   | No       | The export format of the DATETIME data type in MySQL mode.  |          |          |
| --timestamp-value-format      | No   | The export format of the TIMESTAMP data type in MySQL or Oracle mode.  |          |          |
| --timestamp-tz-value-format   | No   | The export format of the TIMESTAMP WITH TIME ZONE data type in Oracle mode.  |          |          |
| --timestamp-ltz-value-format  | No   | The export format of the TIMESTAMP WITH LOCAL TIME ZONE data type in Oracle mode.  |          |          |
| --exclude-virtual-columns     | No   | Specifies not to export the data of generated columns. By default, the data of generated columns is exported.  |          |          |
...
* --exclude-column-names **'** ***column_name [, column_name...]*** **'** 
  ...
  Excludes the specified columns from the export of table data. Fuzzy match on column names is not supported. To specify multiple tables, separate the table names with commas (,). Table names of tenants in Oracle mode are uppercase by default, while table names of tenants in MySQL mode are lowercase by default. Example: In OceanBase Oracle mode, `--table 'test'` and `--table 'TEST'` represent the TEST table. In OceanBase MySQL mode, `--table 'test'` and `--table 'TEST'` represent the test table. If case sensitivity is required, you can enclose the table name in square brackets ([ ]). Example: `--table '[test]'` represents the test table, and `--table '[TEST]'` represents the TEST table. If the table name is specified as an asterisk (*), it means to import all table definitions or table data.
  > **Notice**  
  > This option will only take effect if the order of fields in the data file is consistent with the order of fields in the table structure after excluding the specified column.
  > In the imported data file, there is no data in the excluded specified column.
