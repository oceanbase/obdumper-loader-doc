...
* --remap-table *'**[source_schema.]source_table:target_table**'*
  ...
  The target can contain only the table name, without a database or schema name. The database or schema that contains the target table is determined by `-D/--database` or a target object selection condition that includes a database name.
