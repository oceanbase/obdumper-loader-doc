...
* --date-format
  ...
  The date and time format of the original data. The default value is yyyy-MM-dd HH:mm:ss. It is used to verify whether the data in the file is valid date and time data. If the date and time data is invalid, the value specified by `--default date` is used as the default date and time.
...
| --default-date  | No | An invalid date value has no default value. The date value is formatted based on the `--date-format` option. If the formatting fails, the date value specified by `--default date` is returned. | V4.3.4.1  |   |
| --date-format | No | The date and time format of the original data. The default value is yyyy-MM-dd HH:mm:ss. This option is used to verify whether the data in the file is valid date and time data. If the data is invalid date and time data, the value specified by `--default date` is used as the default date and time. | V4.3.4.1  |  |
