|slug|obloader-command-line-options|
|---|---|

# Command-line options

OBLOADER uses command-line options to specify the information required for import. For more information about options and examples, see [Options](#Options) and [Examples](100.obloader-quick-start.md).
...
|-----------------|----------------------|----------------|
| --csv <br> Specifies to import a CSV file. | <ul><li>--skip-header<br>Specifies whether to skip the header row of a CSV file. The value is BOOLEAN. </li><li>--column-delimiter '***column_delimiter_char***'<br>Specifies the string delimiter, by default a single quotation mark ('). This option is equivalent to `--column-quote` and both cannot be used simultaneously. <main id="notice" type='explain'><h4>Note</h4><p> <code>--column-delimiter</code> is deprecated and may be removed in later versions. We recommend that you use <code>--column-quote</code> instead.  </p></main> </li><li>--column-quote '***column_quote_char***'<br>Specifies the string wrapper, by default a single quotation mark ('). This option is equivalent to `--column-delimiter` and both cannot be used simultaneously. </li><li>--line-separator '***line_separator_string***'<br>Specifies the line separator. </li><li>--escape-character '***escape_char***'<br>Specifies the escape character. </li><li> --column-separator '***column_separator_char***'<br> Specifies the column separator in the CSV format. The default value is a comma (,). </li><li> --with-trim<br>Deletes the left- and right-side spaces. </li><li> --ignore-unhex<br>Indicates whether to ignore the decoding of hexadecimal strings. This option applies only to binary data types and is effective only in the MySQL mode of OceanBase Database.  </li><li> --character-set '***character_set_string***'<br>Specifies the character set for creating a database connection. </li><li>--null-string '***null_replacer_string***'<br>Replaces a specified string with NULL. For example, if the column value in the data file is test and you use the `--null-string test` option, the column will be imported as NULL. </li><li>--empty-string '***empty_replacer_string***'<br>Replaces a specified string with an empty string (' '). The default value is \\E. </li><li>--file-suffix '***suffix_name***'<br>Specifies the file suffix name when importing data. </li><li>--file-encoding '***encode_name***'<br>Specifies the file encoding used to read the data file, which is not the same as the database encoding. </li></ul>|Applicable to almost all business scenarios.|
| --cut <br> Specifies to import a CUT file. | <ul><li>--skip-header<br>Specifies whether to skip the header row of a CUT file. The value is BOOLEAN. </li><li>--skip-footer<br>Specifies whether to skip the last row of a CUT file. </li><li>--column-splitter '***split_string***'<br>Specifies the column separator in the CUT format. </li><li>--trail-delimiter<br>Indicates whether to add a delimiter at the end of each data row. </li><li>--line-separator '***line_separator_string***'<br>Specifies the line separator. </li><li> --with-trim<br>Deletes the left- and right-side spaces. </li><li> --ignore-escape<br>Specifies whether to ignore the escape characters when importing a CUT file. The default value is false. </li><li> --ignore-unhex<br>Indicates whether to ignore the decoding of hexadecimal strings. This option applies only to binary data types and is effective only in the MySQL mode of OceanBase Database.  </li><li> --character-set '***character_set_string***'<br>Specifies the character set for creating a database connection. </li><li>--null-string '***null_replacer_string***'<br>Replaces a specified string with NULL. For example, if the column value in the data file is test and you use the `--null-string test` option, the column will be imported as NULL. </li><li>--escape-character '***escape_char***'<br>Specifies the escape character. </li><li>--empty-string '***empty_replacer_string***'<br>Replaces a specified string with an empty string (' '). The default value is \\E. </li><li>--file-suffix '***suffix_name***'<br>Specifies the file suffix name when importing data. </li><li>--file-encoding '***encode_name***'<br>Specifies the file encoding used to read the data file, which is not the same as the database encoding. </li></ul>|Similar to CSV. <br>Often used to import flexible data exchange formats exported by some older platforms. |
...
* --remap-table *'**[source_schema.]source_table:target_table**'*

  Specifies a source-to-target table mapping to rename the source table. The syntax is as follows:
  ...
  ```shell
  --remap-table orders:orders_bak
  --remap-table old_db.orders:orders_bak
  --remap-table PROD.ORDERS:ORDERS_BAK
  ```

  The following is an invalid example:

  ```shell
  --remap-table old_db.orders:new_db.orders_bak
  ```

  The target can contain only the table name, without a database or schema name. The database or schema that contains the target table is specified by `-D/--database`.

  `--remap-table` defines the complete set of tables to import and cannot be used with `--all`, `--table`, other database object selection options, or object exclusion options.

  You can specify this option multiple times or separate multiple mappings with commas (,). For example:

  ```shell
  --remap-table t1:new_t1 --remap-table t2:new_t2
  ```

  The preceding command is equivalent to:

  ```shell
  --remap-table 't1:new_t1,t2:new_t2'
  ```

  When you use this option, note the following:

  * You must also specify `--ddl`.

  * `-f/--file-path` must point to a native export directory generated by OBDUMPER that contains the DDL files of the tables to be remapped. It cannot point to a single file.

  * This option cannot be used with `--mix`, `--retry`, `--server`, `--replace-object`, or `--hbase-cf`.

  * If the source schema is not specified and the native export directory generated by OBDUMPER contains multiple source tables with the same name, the import fails because the source table cannot be uniquely identified.

  * If multiple source tables are mapped to the same target table, you must also specify `--skip-existing-object`. Otherwise, subsequent table creation statements fail because the target table already exists.

  * Circular foreign key dependencies among tables to be remapped are not supported in the current version.
...
* --ddl<br>Specifies to import DDL files. DDL files can be imported in parallel in OceanBase Database V4.2.1 and later.
  ...
  * --ignore-unhex specifies to ignore the decoding of hexadecimal strings. This option applies only to binary data types and is effective only in OceanBase Database's MySQL compatible mode.
...
* --auto-column-mapping
  ...
  Specifies to enable automatic column mapping. In this mode, table data from the source file will be imported to the target table column by column. The number of columns in the source file can be different from that in the target table. If the source file does not contain a header row, columns are matched by their order. If the source file contains a header row, columns are matched by their names. Supported file formats include CSV, ORC, Parquet, SQL, and AVRO. For an example of how to use this parameter, see [Command-line application example](../500.OBLOADER/310.scene-example.md).

  <main id="notice" type='notice'>
    <h4>Note</h4>
    <ul><li>When you import a CSV file that contains a header row, you must specify the --skip-header option to skip the header row. </li><li>You cannot specify both the --ctl-path and --auto-column-mapping options. </p>
    </main>

* --exclude-object-types

  Specifies to filter out database objects of a specified type during import or export. For example, `--all --exclude-object-types TABLE` specifies to process all objects except tables.
...
* --max-discards ***int_num***
  ...
  <main id="notice" type='notice'>
      <h4>Note</h4>
      <ul>
      <li>This option takes effect only when a table has a primary key or unique key and contains duplicate data. It only applies to errors that occur when writing data to the database, not to other types of errors (such as errors that occur when reading data from the database file). </li>
      <li>This option is not applicable to direct load. </li>
      </ul>
    </main>
...
* --max-errors ***int_num***
  ...
  <main id="notice" type='notice'>
      <h4>Notice</h4>
      <ul>
      <li>This option only applies to errors that occur when writing data to the database, not to other types of errors (such as errors that occur when reading data from the database file).</li>
      <li>This option is not applicable to direct load. </li>
      </ul>
    </main>
...
For more information about the OBKV features, see the **Import data from HBase to OBKV** section in [Quick start](100.obloader-quick-start.md).

* --hbase-cf

  Specifies the column family when data is written to HBase.

* --hbase-rowkey

  Specifies the name of the rowkey column when data is written to HBase.

* --hbase-rowkey-reverse

  Specifies whether to reverse the rowkey column when data is written to HBase.
...
* --skip-existing-object
  ...
  During schema import, if a `CREATE` statement fails because the target object already exists, this option skips the statement and marks the corresponding object as `SKIPPED` in the Summary.

  The `SKIPPED` status means that at least one statement in the object definition file was skipped, not that the entire file was skipped. This option does not compare, update, or replace existing object definitions. It also does not skip subsequent data import.

  <main id="notice" type='notice'>
      <h4>Notice</h4>
      <ul>
      <li>We recommend that you use this option with <code>--ddl</code>.</li>
      <li>This option cannot be used with <code>--replace-object</code>.</li>
      </ul>
  </main>

* --replace-object

  Use this option to replace existing database object definitions when importing database objects. For objects such as tables and synonyms, OBLOADER deletes and then recreates the objects. For objects such as functions and stored procedures, OBLOADER replaces them by using CREATE OR REPLACE statements.

  <main id="notice" type='notice'>
      <h4>Notice</h4>
      <ul>
      <li>This option can be used only with <code>--ddl</code> or <code>--mix</code>. It does not take effect with other data format options such as <code>--csv</code> or <code>--sql</code>.</li>
      <li>This option cannot be used with <code>--skip-existing-object</code>.</li>
...
* --default-date
  ...
  <main id="notice" type='notice'>
      <h4>Notice</h4>
      <p>This parameter takes effect only on the DATE and DATETIME data types in MySQL compatible mode of OceanBase Database. </p>
  </main>
...
| --remap-table             | No    | Specifies a source-to-target table mapping to rename the source table. You can specify this option multiple times or separate multiple mappings with commas (,). | V4.3.7 |          |
...
| --exclude-object-types | No    | The option for excluding object types from import. For example, `--all --exclude-object-types TABLE` processes all objects except TABLE. | V4.3.6    |          |
| --skip-existing-object | No    | Skips a `CREATE` statement during schema import if it fails because the target object already exists, marks the corresponding object as `SKIPPED` in the Summary, and continues with subsequent data import. | V4.3.7 |          |
...
| --skip-header             | No    | A boolean value indicating whether to skip the first line of CSV/CUT/POS files.  Only from V3.3.0, the first line of CUT files can be skipped.           |          |          |
|  --skip-footer                | No    | The option for skipping the last line of CUT files.                        |       V3.3.0   |          |
| --null-string             | No    | The string to replace NULL. Default value: \\N. <main id="notice" type='notice'><h4>Notice</h4><p>`,,` and `,"",` are both parsed as empty strings. Only when the value is \\N is it parsed as NULL. </p></main>                 |          |          |
| --empty-string            | No    | The string to replace NULL. Default value: \\N. <main id="notice" type='notice'><h4>Notice</h4><p>`,,` and `,"",` are both parsed as empty strings. Only when the value is \\N is it parsed as NULL. </p></main>                  |          |         |
| --line-separator          | No    | The line separator. <br>When you import CSV, CUT, POS, or SQL files, you can specify the line separator in the data file. The default value of this option depends on the system platform and includes only three possible values: ***\\r***, ***\\n***, and ***\\r\\n***.         |          |          |
| --column-separator        | No    | The column separator in CSV files, which is different from the column separator in CUT files. This option can be used only with the `--csv` option. Default value: comma (,).  <main id="notice" type='notice'><h4>Notice</h4><p>This option supports only single characters in versions earlier than OBLOADER V4.3.5. </main>               |          |          |
| --escape-character        | No    | The escape character. This option supports only single characters and can be used only with the `--csv` or `--cut` option. <ul><li>When used with the `--csv` option, the default value is null. </li><li>When used with the `--cut` option, the default value is \\.</li> </ul><main id="notice" type='explain'><h4>Note</h4><p>If you specify this option for a special (invisible) character, use its hexadecimal representation. For example, you can use '\x09' to represent an invisible tab character. </p></main>                        |          |          |
| --column-delimiter        | No    | The string delimiter. Default value: single quotation mark('.'). This option is equivalent to `--column-quote` and cannot be specified with it. This option supports only single characters and can be used only with the `--csv` option. <main id="notice" type='explain'><h4>Note</h4><p><code>--column-delimiter</code> is an obsolete parameter and may be removed in later versions. We recommend that you use <code>--column-quote</code> instead. </p></main>                            |          |          |
| --column-quote        | No    | The string delimiter. Default value: single quotation mark('.'). This option is equivalent to `--column-delimiter` and cannot be specified with it. This option supports only single characters and can be used only with the `--csv` option.                           |    V4.3.5      |          |
| --ignore-unhex            | No    | The option for ignoring the decoding of hexadecimal strings. This option applies only to binary data types and takes effect only in the MySQL compatible mode of OceanBase Database.                          |          |          |
...
| --max-discards            | No    | The maximum number of duplicate records allowed for a single table during import. Default value: -1. <main id="notice" type='notice'><h4>Notice</h4><ul><li>This option takes effect only when a table has a primary key or unique key and contains duplicate data. It only applies to errors that occur when writing data to the database, not to other types of errors (such as errors that occur when reading data from the database file).</li><li>This option is not applicable to direct load. </li></ul></main>                   |          |          |
| --max-errors              | No    | The maximum number of errors allowed for a single table during import. Default value: 1000.  <main id="notice" type='notice'><h4>Notice</h4><ul><li>This option only applies to errors that occur when writing data to the database, not to other types of errors (such as errors that occur when reading data from the database file).</li><li>This option is not applicable to direct load. </li></ul></main>                   |          |          |
...
|  --file-regular-expression                |  No   | The option for specifying the regular expression of file names to selectively import files.                     |       V3.3.0 <main id="notice" type='explain'><h4>Note</h4><p>V4.3.2 and later support the import of files by using regular expressions. </p></main>|          |
