...
OBLOADER uses command-line options to specify the information required for import. For more information about options and examples, see **Option list** and [Examples](../500.OBLOADER/100.obloader-quick-start.md).
...
OBLOADER V4.3.2 and later versions support importing multiple tables in a database by specifying matching rules. You can use the group names (including `schema` and `table`) to specify the database name and table names. If only the schema group name is specified, the `--table` option must specify a single table or a database with only one table, otherwise OBLOADER will exit. The following rules are supported.
...
OBLOADER V4.2.6 and later support the bypass import mode. This feature supports OceanBase Database V4.2.1.7 and later in the V4.2.1.x series, V4.3.0.1 and later in the V4.3.0.x series, and V4.2.4.x.

* --rpc-port= ***rpc_port_num***

  Specifies the RPC port of the OBServer node. This option is used in conjunction with `--direct` and `--parallel` to connect to the RPC port of the OBServer node for data import in bypass mode.
...
* --direct
  ...
  Enables the bypass import mode. This option is used in conjunction with `--rpc-port` and `--parallel`.

  `--direct` uses different bypass modes depending on whether the target table contains data.

  * When the target table is empty, `--direct` uses the full bypass mode. It can also be used with `--replace-data`, which specifies that data conflicts should be resolved by `replace`.

  * When the target table is not empty, `--direct` uses the incremental bypass mode. If the target table has no indexes or LOB fields, `--replace-data` can be used. If the target table has indexes or LOB fields, `--replace-data` cannot be used.

  <main id="notice" type='explain'>
    <h4>Note</h4>
    <ul>
    <li> The binary data types are not supported in the OBLOADER bypass import mode. </li>
    <li> The OBLOADER bypass import mode supports connecting to OBServer and ODP. The versions must meet the following requirements: <ul><li>When connecting to OBServer, OBServer must be V4.2.0 or later. </li><li>When connecting to ODP, ODP must be V4.3.0 or later, and OBServer must be V4.2.1 or later. </li></ul></li>
    </ul>
  </main>

* --parallel= ***parallel_num***

  Specifies the degree of parallelism for loading data in bypass mode. This option is used in conjunction with `--rpc-port` and `--direct`.
...
#### Import a file exported by using OBDUMPER

DevelveDB V4.3.0 and later support the import of files exported by using OBDUMPER. Supported importable file formats include CSV, CUT, POS, and SQL. You can configure related settings through the following command-line options.
...
* -f '***file_path***', --file-path= '***file_path***'
  ...
  OBLOADER can import database object definitions and table data from local disks, [Aliyun OSS](https://help.aliyun.com/document_detail/31817.html), Amazon S3, Apache Hadoop, [Tencent COS](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb), [Huawei OBS](https://docs.aws.amazon.com/zh_cn/s3/index.html), Azure Blob, and GCS. The syntax is as follows:
  ...
  |----------------------------------|--------------------------------------------------------|
  |  scheme | The supported storage types. Valid values: [OSS](url-placeholder-2339683f-bf34-4d97-97d9-ea2afd365da9), [S3](url-placeholder-9083d18f-1816-4c93-8489-c81f79a56a3d), Hadoop, [COS](url-placeholder-d69b1434-9430-4b01-9991-f551c84bc4dd), and [OBS](url-placeholder-9083d18f-1816-4c93-8489-c81f79a56a3d).<br> If the specified scheme is not a supported OSS, S3, Hadoop, COS, or OBS, an error occurs.|
  | host | The name of the storage space. <ul><li>When you import data from OSS, Amazon S3, Tencent Cloud Object Storage, and Huawei Cloud Object Storage Service, host specifies the bucket name. For more information, see [OSS Bucket](url-placeholder-d69b1434-9430-4b01-9991-f551c84bc4dd). </li><li>When you import data from Hadoop, host specifies the IP address or cluster name of the Hadoop node, in the format of `<ip>:<port>` or `<cluster_name>`. </li></ul> |
  ...
  |**Parameter** |**Required**|**Description**|**Supported storage type**|**Supported version**|
  |--------|------|------|------|------|
  | endpoint  |Yes |<ul><li>The endpoint of the region where the OSS host is located. </li><li>To access Amazon S3, Tencent Cloud Object Storage, and Huawei Cloud Object Storage Service, specify the endpoint by domain name. </li></ul>|OSS, S3, COS, and OBS|<ul><li>OSS: V4.2.0 </li><li>S3: V4.2.5 </li><li>COS and OBS: V4.3.0 </li></ul>|
  | region |Yes |The endpoint specifies the physical location of the bucket. |S3, COS, and OBS|<ul><li>S3: V4.2.0 </li><li>COS and OBS: V4.3.0 </li></ul>|
...
* --replace-data
  ...
  Specifies to replace the duplicate data in the table. This option applies only to tables with defined primary keys or unique keys (including non-null fields). If the file contains a large amount of duplicate data, which exceeds 30% of the total data, we recommend that you clear the table and start data import again. Data replacement is less efficient than data import into an empty table. This option takes effect only when it is used together with the `--csv`, `--sql`, or `--cut` option, but not with the `--ddl` option.
...
| -t                        | No    | The tenant name for connecting to OceanBase Database. When you perform direct load in the cloud, you must use the <code>--public-cloud -t <tenant></code> option. <main id="notice" type='alert'><h4>Notice</h4><p>When performing direct load in the cloud, you must use the <code>--public-cloud -t <tenant></code> option. </p></main> |    |    |
| --public-cloud            | No    | The option for importing database objects or table data from an OceanBase cluster deployed in the cloud. <main id="notice" type='alert'><h4>Notice</h4><p>When performing direct load in the cloud, you must use the <code>--public-cloud -t <tenant></code> option. </p></main>                                     |          |          |
