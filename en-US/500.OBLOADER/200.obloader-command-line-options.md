...
* --skip-existing-object
  ...
  Automatically skips objects that already exist in the target database during schema import and displays the skipped objects with the `SKIPPED` status in the Summary. This option applies to incremental schema import or rerunning an interrupted task. It prevents import failures caused by existing objects.
...
| --skip-existing-object | No    | Automatically skips objects that already exist in the target database during schema import and displays the skipped objects with the `SKIPPED` status in the Summary. | V4.3.7 |          |
