...
* OBLOADER V4.3.2 and later versions support the `--mem` parameter for specifying the memory size of the JVM. Valid units are K, M, G, and T. The default value is 4G.
...
You can also use the `--mem` parameter to specify the memory size of the VM instead of modifying the VM parameter. For example, `--mem 4G`.
