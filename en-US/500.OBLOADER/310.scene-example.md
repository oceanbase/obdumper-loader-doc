|slug|use-cases-of-command-line-options|
|---|---|
...
When you specify the `--skip-existing-object` option for schema import, OBLOADER skips a `CREATE` statement that fails because the target object already exists and then continues with the remaining statements in the file and any subsequent data import. This option is useful for incremental schema imports and for rerunning interrupted tasks.

### Scenario

Import the definitions of the `customer_orders` and `customers` tables in the `/path/to/ddl` directory into the `cities_new` database. The `customer_orders` table already exists in the target database, whereas the `customers` table does not.

### Procedure

1. Run the following command to import the object definitions:

    ```shell
    ./bin/obloader -h xxx.xxx.xxx.xxx -P 2883 -u test@mysql#cluster_a -p ****** -D cities_new --ddl --table 'customer_orders,customers' -f /path/to/ddl --skip-existing-object
    ```

2. Wait for the command to finish. The `CREATE TABLE` statement for `customer_orders` is skipped, leaving the existing table definition unchanged. In the Summary, `customer_orders` is shown with the `SKIPPED` status. The `customers` table is created as expected.
