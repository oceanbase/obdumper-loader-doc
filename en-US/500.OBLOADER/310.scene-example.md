...
When you specify the `--skip-existing-object` option for schema import, OBLOADER automatically skips objects that already exist in the target database. This option applies to incremental schema import or rerunning an interrupted task. It prevents import failures caused by existing objects.
...
2. Wait for the command to finish. The `customer_orders` table is skipped and displayed with the `SKIPPED` status in the Summary. The `customers` table is imported as expected.
