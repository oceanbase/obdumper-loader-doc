...
|---|---|
|machine-translation||
|slug|obloader-define-control-files|

# Define a control file

With a defined control file, OBLOADER can [preprocess](../300.obloader-data-processing/200.obloader-preprocessing-functions.md) the imported data. For information about control file activation rules, column mapping, fixed-length POS files, generated columns, common use cases, and troubleshooting, see [Control file guide](../../650.control-file-guide.md).
...
* Column name: the name of a field in the database table structure. In MySQL compatible mode, control file column names are always matched in a case-insensitive manner. In Oracle compatible mode, column names that are not enclosed in square brackets or backticks are normalized to uppercase before matching. Column names enclosed in square brackets or backticks preserve their case and must exactly match the database column names. For example, `[c1]` indicates the c1 column, and `[C1]` indicates the C1 column.
...
* Mapping definition: the position of the column that stores preprocessed data in the data file.
  ...
  <main id="notice" type='notice'>
    ...
    <li>Before data is imported, OBLOADER retrieves control files with the <code>.ctrl</code> extension and preprocesses the data based on the functions specified in the control files. <ul><li>When multiple tables are imported with multiple control files, OBLOADER recursively retrieves all files with the <code>.ctrl</code> extension from the <code>ctl-path</code> directory and its subdirectories, and binds them to the imported tables by matching the control file base names with the table names. In MySQL compatible mode, matching follows the <code>lower_case_table_names</code> setting of the target database. Matching is case-sensitive when the value is <code>0</code> and case-insensitive when the value is <code>1</code> or <code>2</code>. In Oracle compatible mode, unquoted table names are normalized to uppercase before matching.</li><li>When only one table is imported and <code>--ctl-path</code> points directly to a control file, OBLOADER binds the file to the table without validating the control file name against the table name.</li></ul></li>
    </ul>
  </main>
