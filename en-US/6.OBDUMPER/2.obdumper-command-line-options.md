...
* --timestamp-tz-value-format
   ...
   The export format of the TIMESTAMP WITH TIME ZONE data type in Oracle mode. By default, the data type is exported in the yyyy-MM-dd HH:mm:ss TZR format. The precision is the same as that in the table definition.
<br><br>
...
* --block-size ***int_num***
   ...
   The block size for a file to be exported. If the size of the data file to be exported exceeds the value of this option, the file will be split into blocks with this specified size. Default value: 1GB. This option does not take effect for ORC and Parquet formats.
<br><br>
