...
* --sys-user ***sys_username***
   ...
   The username of a user with required privileges under the sys tenant, for example, root or proxyro. OBLOADER requires a special user under the sys tenant to query the metadata in system tables. Default value: root. You do not need to specify this option for OceanBase Database V4.0.0 and later versions.
<br><br>
