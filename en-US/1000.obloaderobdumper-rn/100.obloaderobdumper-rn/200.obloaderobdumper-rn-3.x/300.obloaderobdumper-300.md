...
|-------------------------|--------------------------------------------------------------------------------------------------------|
| OceanBase Database in the Oracle compatible mode | V2.0.x, V2.1.x, V2.2.20, V2.2.30, V2.2.50, V2.2.70, V2.2.71, V2.2.72, V2.2.76, V2.2.77, V3.1.x, and V3.2.x                     |
| OceanBase Database in the MySQL compatible mode  | V1.4.70, V1.4.72, V1.4.75, V1.4.78, V1.4.79, V2.2.30, V2.2.50, V2.2.70, V2.2.71, V2.2.72, V2.2.76, V2.2.77, V3.1.x, and V3.2.x |
...
* OBLOADER and OBDUMPER: The object names, data file names, and rule file names specified by command-line options must be capitalized in the same way. By default, uppercase letters are used in the Oracle compatible mode and lowercase letters are used in the MySQL compatible mode.
...
* OBLOADER:
  ...
  * OceanBase Database in the Oracle compatible mode does not support the import and export of virtual column data. This issue is fixed in the MySQL compatible mode where virtual columns are ignored during data export. Otherwise, virtual data cannot be imported.

  * In the MySQL compatible mode of OceanBase Database V1.4.x, the metadata of the RANGE_COLUMNS + KEY is defective in the virtual routing view.
...
* OBDUMPER:
  ...
  * OceanBase Database V1.4.72 in the MySQL compatible mode does not allow you to export the definitions of unique prefix indexes. Example: `UNIQUE(c1(10))`.

  * When you resolve the primary key conflict in OceanBase Database V1.4.79 in the MySQL compatible mode, the use of the `insert ... where not exists` statement may result in cross-partition insertion.

  * OceanBase Database of versions earlier than V2.2.50 in the Oracle compatible mode do not allow you to export the DDL statements of function-based indexes.

  * OceanBase Database V2.2.7x in the MySQL compatible mode does not allow you to export vertical partitions. For example, partitions created by using the PARTITION BY COLUMN statement cannot be exported.

  * OceanBase Database V2.2.7x in the MySQL compatible mode does not allow you to export synonyms. For example, synonyms created by using the CREATE SYNONYM statement cannot be exported.

  * OceanBase Database V2.2.7x in the MySQL compatible mode does not allow you to export columns that store index data. For example, columns in STORING(COLUMN_LIST) cannot be exported.

  * Data of the INTERVAL DAY(2) TO SECOND(0) type cannot be exported. For example, '0 5:0:0.000000' cannot be exported.

  * In the Oracle compatible mode, data of the NUMBER(n,m) type and data less than 1 are exported in scientific notation. For example, 0.0000007 is exported as 7E-7.
