...
* OBLOADER:
  ...
  * The `--direct`, `--rpc-port`, and `--parallel` options are provided so that you can connect to an OBServer node to import data in the direct load mode. For more information, see [Command-line options of OBLOADER](../../../500.OBLOADER/200.obloader-command-line-options.md).
...
* OBLOADER and OBDUMPER:
  ...
  * The connection configuration file `session.properties` is replaced with `session.config.json` so that you can manage session parameters more flexibly. For more information, see [Connection configuration](../../../800.obloaderobdumper-session-variables.md).

  * You can configure the JDBC high-availability mode in the following ways:

    * Method 1: Specify the [jdbc_hamode](../../../800.obloaderobdumper-session-variables.md) parameter in the `session.config.json` file. For more information, see [Failover and load balancing modes](https://en.oceanbase.com/docs/common-oceanbase-connector-j-en-10000000002244941).
...
* Fixed the issue where the table is locked and data import is affected when OBLOADER becomes abnormal or forcibly exits due to an exception during direct load.
...
* Fixed the issue where the export fails if the name of a sequence is in lowercase when OBDUMPER is used to export data from OceanBase Database in Oracle compatible mode.
...
* If you do not specify the password of the sys tenant on the command line, OBDUMPER cannot export unique index definitions of partitioned tables from OceanBase Database V2.2.70 and later in Oracle compatible mode.
