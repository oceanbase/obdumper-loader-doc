...
OBLOADER & OBDUMPER V4.3.1.1 released in August 2024 fixes some known issues as well as the issue where no error is returned for incomplete export of non-partitioned tables without primary keys.
...
* Fixed the issue where data cannot be imported or exported in MySQL compatible mode of OceanBase Database when `lower_case_table_names` is set to `0` and the database name contains uppercase letters.

* Fixed the issue where the export of datetime types from partitioned tables fails in Oracle compatible mode of OceanBase Database later than V4.0.0.0.
...
* Fixed the issue where the size of exported historical data of partitioned tables doubles in Oracle compatible mode of OceanBase Database V3.2.x.
