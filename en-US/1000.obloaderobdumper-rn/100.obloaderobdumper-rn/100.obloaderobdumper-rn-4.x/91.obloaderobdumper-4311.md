...
obloader & obdumper V4.3.1.1 was released in August 2024 to fix some known issues as well as the issue where no error is returned for incomplete export of non-partitioned tables without primary keys.

## Version information

* Current version: V4.3.1.1
...
The following table lists the OceanBase Database versions supported by obloader & obdumper V4.3.1.1.
...
* Fixed the issue where data cannot be imported or exported in a MySQL tenant of OceanBase Database when `lower_case_table_names` is set to `0` and the database name contains uppercase letters.

* Fixed the issue where the export of datetime types from partitioned tables fails in Oracle tenants of OceanBase Database later than V4.0.0.0.

* Fixed the issue where the following error is returned during the export of a table without a primary key: `Current position is after the last row`.

* Fixed the issue where the task to export data from a non-partitioned table without a primary key ends before all data is exported.

* Fixed the issue where the size of exported historical data of partitioned tables doubles in Oracle tenants of OceanBase Database V3.2.x.
