...
OBLOADER & OBDUMPER V4.2.8 released in January 2024 supports preprocessing functions related to SM4-based encryption and decryption, date and time, and data export in fixed-length formats. This version further allows you to export table schema definitions by using the `SHOW CREATE TABLE` statement in OceanBase Database V4.x and export files with fixed-length fields by using the `--pos` option. This version improves the metadata query efficiency in the Oracle compatible mode of OceanBase database as well as the user experience of various command-line options.
...
* Schema export performance optimization is supported for OceanBase Database in the Oracle compatible mode in the following scenarios:

  * The `--no-sys` option is used in the Oracle compatible mode of OceanBase Database of a version earlier than V4.0.0.

  * The `--compact-schema` option is used in the Oracle compatible mode of OceanBase Database V4.0.0 and later.
...
* If you do not specify the password of the sys tenant on the command line, OBDUMPER cannot export unique index definitions of partitioned tables from OceanBase Database V2.2.70 and later in the Oracle compatible mode.
