...
|---|---|
|machine-translation||
|slug|obloaderobdumper-4.2.8|

# OBLOADER & OBDUMPER V4.2.8

OBLOADER & OBDUMPER V4.2.8 released in January 2024 supports preprocessing functions related to SM4-based encryption and decryption, date and time, and data export in fixed-length formats. This version further allows you to export table schema definitions by using the `SHOW CREATE TABLE` statement in OceanBase Database V4.x and export files with fixed-length fields by using the `--pos` option. This version improves the metadata query efficiency in Oracle compatible mode of OceanBase database as well as the user experience of various command-line options.
...
* Schema export performance optimization is supported for OceanBase Database in Oracle compatible mode in the following scenarios:

  * The `--no-sys` option is used in Oracle compatible mode of OceanBase Database of a version earlier than V4.0.0.

  * The `--compact-schema` option is used in Oracle compatible mode of OceanBase Database V4.0.0 and later.
...
* OBLOADER:
  ...
  * The following preprocessing functions are supported:
    ...
    * `DATE_TRUNC`: truncates values of the DATE type to a specific field. For example, you can use this function to truncate values of a specific column to the day, month, or year.
    ...
    * `DAY_OF_WEEK`: returns the day of the week as an uppercase English string from `MONDAY` to `SUNDAY`.
    ...
    * `MINUTE`: returns the minute (0-59) for values of the TIMESTAMP type.

    * `MONTH`: returns the month as an uppercase English string from `JANUARY` to `DECEMBER`.
