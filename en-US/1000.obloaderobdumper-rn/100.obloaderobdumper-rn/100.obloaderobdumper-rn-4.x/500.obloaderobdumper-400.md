...
* OBDUMPER reports a syntax error for the export of partition indexes from OceanBase Database V2.2.30 in the Oracle compatible mode.
...
* OBDUMPER cannot export public synonyms from OceanBase Database in the Oracle compatible mode.
...
* The column comments exported from OceanBase Database in the Oracle compatible mode have a syntax error when the sys tenant password is not specified on the OBDUMPER command line.
...
* OBDUMPER cannot export sequence definitions from OceanBase Database in the MySQL compatible mode.
