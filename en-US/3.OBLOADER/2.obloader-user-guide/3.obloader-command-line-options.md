...
| --mix                     | No        | Imports files that contain both definitions and data.                                                                                                 |                   |                |
...
* --table ' ***table_name [,table_name...]*** ' | --table ' **\*** '
  ...
  Imports table definitions or table data. When this option is used in combination with `--ddl`, only table definitions are imported. When this option is used in combination with any data format option, only table data is imported. To specify multiple tables, separate the table names with commas (,). Table names of tenants in Oracle mode are uppercase by default, while table names of tenants in MySQL mode are lowercase by default. Example: In OceanBase Oracle mode, `--table 'test'` and `--table 'TEST'` represent the TEST table. In OceanBase MySQL mode, `--table 'test'` and `--table 'TEST'` represent the test table. If case sensitivity is required, you can enclose the table name in square brackets ([ ]). Example: `--table '[test]'` represents the test table, and `--table '[TEST]'` represents the TEST table. If the table name is specified as an asterisk (*), it means to import all table definitions or table data.
<br><br>
...
* --ingore-unhex
  ...
  Ignores hexadecimal strings in decoding. This option is applicable to binary data types.
<br><br>
...
* --exclude-column-names **'** ***column_name [, column_name...]*** **'** 
  ...
  Excludes the specified columns from the import of table data. Fuzzy match on column names is not supported. To specify multiple tables, separate the table names with commas (,). Table names of tenants in Oracle mode are uppercase by default, while table names of tenants in MySQL mode are lowercase by default. Example: In OceanBase Oracle mode, `--table 'test'` and `--table 'TEST'` represent the TEST table. In OceanBase MySQL mode, `--table 'test'` and `--table 'TEST'` represent the test table. If case sensitivity is required, you can enclose the table name in square brackets ([ ]). Example: `--table '[test]'` represents the test table, and `--table '[TEST]'` represents the TEST table. If the table name is specified as an asterisk (*), it means to import all table definitions or table data.
  > **Notice**  
...
* --truncate-table
  ...
  > **Notice**  
  > <li> If the `--all` or `--table '*'` option is used in combination with the `--truncate-table` option, the tool truncates all tables in the destination database, even if there are no corresponding data files of the tables in the directory specified by the `-f` option.</li>
...
* --delete-from-table 
  ...
  > **Notice**  
  > <li> If the `--all` or `--table '*'` option is used in combination with the `--delete-from-table` option, the tool clears all tables in the destination database, even if there are no data files of the tables in the directory specified by the `-f` option.</li>
