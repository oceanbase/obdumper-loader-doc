...
| TO_TIMESTAMP(char,fmt1[,fmt2])                                                  | String   | 对参数值进行日期格式化。仅当 `char` 为 `NULL` 时返回 `NULL`；非空值转换失败时抛出异常。导入过程中，该记录会被标记为 bad 并写入 `ob-loader-dumper.bad`，任务是否失败取决于最大错误数等配置。参数 `char` 的值可以是列名、常量或函数表达式，参数 `fmt1` 的值是日期解析模板，可选参数 `fmt2` 的值是日期格式化模板。<br>参数 `fmt2` 的默认值：yyyy-MM-dd HH:mm:ss.SSS。               |
...
|DB_SEQUENCE(string sequence)|String|指定数据库中已定义的序列导入数据。序列名必须使用单引号。<br>示例：`DB_SEQUENCE('seq01')`，指定序列值为 seq01。<blockquote>**说明**</br>OBLOADER 4.1.0 及之后的版本支持该功能。</blockquote>|
...
|sm3_digest (plaintext [,encoding])| String |使用 SM3 算法对数据进行哈希，并返回提取的摘要。函数参数：<ul><li>plaintext：表示原始文本</li><li>encoding：表示生成摘要的编码类型。支持 Base64 与 HEX，默认值为 HEX。</li></ul> 示例：c1 "sm3_digest(c1)" <br>c2 "sm3_digest(c2, 'BASE64')"|

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OBLOADER V3.x 及之后版本不再支持 groovy 动态函数。</p>
  </main>
