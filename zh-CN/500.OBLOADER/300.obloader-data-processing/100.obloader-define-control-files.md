...
通过已定义的控制文件，OBLOADER 可以对被导入的数据进行[预处理](../300.obloader-data-processing/200.obloader-preprocessing-functions.md)。有关控制文件的生效规则、列映射、POS 定长文件、生成列、常用场景和问题排查，请参见[控制文件使用指南](../../650.control-file-guide.md)。
...
* 列名：数据库表结构中的字段名称。MySQL 模式下，OBLOADER 始终不区分控制文件列名的大小写。Oracle 模式下，未使用中括号或反引号的列名按大写归一化后匹配；使用中括号或反引号时保留大小写，必须与数据库列名精确匹配。例如：`[c1]` 表示 c1 列，`[C1]` 表示 C1 列。

* 字节偏移位置：支持两种声明方式，绝对偏移和相对偏移。仅 `--pos` 格式数据支持定义字节偏移位置。

  * 绝对偏移：`position(start, end)`，`start` 与 `end` 分别表示字段的起始位置（字节）与结束位置（字节）。如果需要指定列长以忽略导入某一段字节到数据库表中，您可以使用特殊关键字 _FILLER 代替实际的列名并标记该部分数据，此操作将创建一个匿名列，该列会由 OBLOADER 解析，但不会与表中的任何现有列关联。此关键字特别适用于忽略文件中某些不相关的数据，如行尾的填充字节。示例：<code>_filler position(5:10)</code>，表示跳过第 5 到第 10 个字节。
  
  * 相对偏移：`position(length)`，`length` 表示字段的长度（字节）。

* 预处理函数：在控制文件中为指定的列配置[预处理函数](../300.obloader-data-processing/200.obloader-preprocessing-functions.md)，以对导入的数据进行预处理。
...
* 映射定义：导入的数据文件中预处理数据的列位置。
  ...
  <main id="notice" type='notice'>
    ...
    <li>数据导入前，OBLOADER 会检索以 <code>.ctrl</code> 结尾的控制文件，并根据控制文件中配置的函数对导入的数据进行预处理。<ul><li>导入多个表且包含<strong>多个控制文件</strong>时，OBLOADER 会递归检索 <code>ctl-path</code> 路径下所有以 <code>.ctrl</code> 结尾的文件，并按照控制文件基本名与表名的对应关系绑定到导入的表。MySQL 模式下，匹配规则由目标数据库的 <code>lower_case_table_names</code> 变量决定：值为 <code>0</code> 时区分大小写，值为 <code>1</code> 或 <code>2</code> 时不区分大小写。Oracle 模式下，未引用的表名按大写归一化后参与匹配。</li><li>只导入一张表且 <code>--ctl-path</code> 直接指向一个控制文件时，OBLOADER 会直接将该文件绑定到导入的表，不校验控制文件名与表名。</li></ul></li>
    </ul>
  </main>
...
* 控制文件不能和 `--exclude-column-names` 一起生效，控制文件包含 `--exclude-column-names` 的功能。

* 控制文件中需要列出数据库目标表结构中的字段名称，否则无法与数据文件字段值对应。
...
2. 定义控制文件。用户在控制文件中为被导入的每一列配置对应的 [预处理函数](../300.obloader-data-processing/200.obloader-preprocessing-functions.md)。
   ...
   **示例语句**：
   ...
   (
     ...
     c11 "lpad(c11,5,'x')" map(11),                                                                    -- c11 列的值左侧追加 5 个字符长度字符串 'x'
     c12 "rpad(c12,5,'x')" map(12),                                                                    -- c12 列的值右侧追加 5 个字符长度字符串 'x'
     ...
     c18 "lpadb(c18,5,'x')" map(18),                                                                   -- c18 列的值左侧追加 5 个字节长度（单）字符 'x'
     c19 "rpadb(c19,5,'x')" map(19),                                                                   -- c19 列的值右侧追加 5 个字节长度（单）字符 'x'
...
3. 通过 `--ctl-path` 选项指定控制文件所在的绝对路径进行数据预处理后，导入 `--csv` 格式的数据。
...
**定义控制文件示例语句**：
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OBLOADER V3.0.0 之后的版本不再支持 <code>--null-replacer</code>、<code>--empty-replacer</code>、<code>--default-date</code> 和 <code>--date-format</code> 选项。</p>
  </main>
