...
OBLOADER 的性能可从命令行选项、JVM 内存和数据库内核等三个方面进行调优。
...
* OBLOADER V4.3.2 及之后版本支持 `--mem` 参数，用于标识 JVM 的内存大小配置。支持的单位包括 K、M、G 或 T，默认值为 4G。

## JVM 参数调优

请将导入脚本中的 JVM 参数修改为可用物理内存的 60%，默认值为 `-Xms4G -Xmx4G`。
...
您也可以通过 `--mem` 参数指定 JVM 内存的大小，替代上述修改 JVM 参数的方式。例如，`--mem 4G`。
...
| alter system set merge_thread_count=32; | 0 | 用于设置每日合并工作的线程数。该配置项的值为 `0` 时，合并的工作进程数的计算方式为 `min{10,cpu_cnt}`，其中 `cpu_cnt` 为系统 CPU 的数量。修改动态参数后，无需重启，即刻生效。<br> 取值范围：[0, 256]。 <main id="notice" type='notice'><h4>注意</h4><p>OceanBase 数据库 V4.0.0 及之后的版本已弃用该参数。</p></main> |
| alter system set minor_merge_concurrency=48; | 0 | 用于设置小合并时的并发线程数。<br> 取值范围：[0, 64]。 <main id="notice" type='notice'><h4>注意</h4><p>OceanBase 数据库 V4.0.0 及之后的版本已弃用该参数。</p></main> |
| alter system set writing_throttling_trigger_percentage=100; | 100 | 设定服务端内存限流阈值（建议禁止打开服务端限流）。<main id="notice" type='notice'><h4>注意</h4><p>OceanBase 数据库 V2.2.30 及之后的版本才支持该系统参数，要求工具拥有防导爆能力。</p></main> |
