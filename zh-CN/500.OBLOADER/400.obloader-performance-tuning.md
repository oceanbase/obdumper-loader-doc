|slug|obloader-performance-tuning|
|---|---|
...
* OBLOADER V4.3.2 及之后版本支持 `--mem` 参数，用于标识虚拟机的内存大小配置。支持的单位包括 K、M、G 或 T，默认值为 4G。

## 虚拟机参数调优

请将导入脚本中的虚拟机参数修改为可用物理内存的 60%，默认值为 `-Xms4G -Xmx4G`。
...
您也可以通过 `--mem` 参数指定虚拟机内存的大小，替代上述修改虚拟机参数的方式。例如，`--mem 4G`。
