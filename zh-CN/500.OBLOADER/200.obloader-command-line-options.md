...
导数工具 V4.3.0 及之后版本，支持解压缩导入 CSV、CUT、POS 和 SQL 等可读文件格式，您可以通过下述命令行选项进行相关配置。
...
* -f '***file_path***', --file-path= '***file_path***'
  ...
  导数工具 V4.3.0 及之后版本支持在腾讯云 COS 和华为云 OBS 上进行导入，`-f`/`--file-path` 新增可接受的 URI Scheme。示例如下：

  * 腾讯云 COS

    ```shell
    cos://mybucket/foo?region=ap-shanghai&access-key=myak&secret-key=mysk
    cos://mybucket/foo?endpoint=yourendpoint&access-key=myak&secret-key=mysk
    ```

  * 华为云 OBS

    ```shell
    obs://mybucket/foo?region=cn-north-1&access-key=myak&secret-key=mysk
    obs://mybucket/foo?endpoint=yourendpoint&access-key=myak&secret-key=mysk
    ```

* --storage-uri '***storage_uri_string***'

  用于标识存储的统一资源定位符。OBLOADER V4.2.0 及之后的版本支持从 Aliyun OSS 或者 Amazon S3 导入数据库对象定义和表数据；OBLOADER V4.2.1 及之后的版本支持从 Hadoop 导入数据库对象定义和表数据。
  ...
  |--------|------|------|------|------|
  | endpoint  |是 |<ul><li>用于指定 OSS host 所在地域的 Endpoint。</li><li>用于通过域名 Endpoin 访问 S3。</li></ul>|<ul><li>OSS</li><li>S3</li></ul>|<ul><li>V4.2.0</li><li>V4.2.5</li></ul>|
  | region |是 |用于指定终端节点，为 S3 bucket 所在的物理位置。|S3|V4.2.0|
  | storage-class |是 |用于指定 Amazon S3 存储类。|S3|V4.2.0|
  | access-key  |是 |用于指定存储的访问账号。|OSS/S3|V4.2.0|
  | secret-key  |是 |用于指定存储的访问密钥。|OSS/S3|V4.2.0|
...
|   --storage-uri               | 否    |     标识存储的统一资源定位符。                     |       V4.2.0   |          |
