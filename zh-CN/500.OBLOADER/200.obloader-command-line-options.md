...
|   --strict               | 否    |     标识导入数据出现错误时结束进程。                     |       4.2.4   |          |
...
* --strict= ***strict_string***
  ...
  用于导入数据出现错误时，导入任务失败并结束进程。默认值：true，表示出现错误时结束进程。false 表示出现错误时跳过错误继续进程。
  
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>该选项可与 <code>--max-discards</code> 或者 <code>--max-errors</code> 选项搭配使用，表示当重复数据量或者错误数在指定范围内，会跳过错误继续进程。更多详情请参见 <a href="../../500.OBLOADER/501.obloader-error-resolution.md">错误处理功能</a></p>
  </main>
