...
<main id="notice" type='explain'>
   <h4>说明</h4>
   <p>导数工具 V4.2.1 及之后的版本不再区分企业版和社区版。用户可以从社区下载中心获取软件包。</p>
</main>
...
JVM 内存太小可能会影响导入导出的性能，甚至影响导入导出功能的稳定性。您可以通过 `--mem` 参数指定 JVM 内存的大小。例如，`--mem 4G`。
...
```shell
./obloader -h 'IP地址' -P'端口' -u'用户'@'租户'#'集群' -p'密码' -D'库名' --table '表名' --csv -f '文件路径' --sys-password '系统租户密码' --external
```
...
**场景描述**：将 `/output` 目录下所有已支持的数据库对象定义导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
```
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><code>--sys-user</code> 选项用于连接 sys 租户下具备特定权限的用户。导入时如果未指定 <code>--sys-user</code> 选项时，默认指定的是 <code>--sys-user root</code>。OceanBase 数据库 V4.0.0 及之后的版本无需指定该选项，详情请参见 <a href="200.obloader-command-line-options.md">OBLOADER 命令行选项</a>。</p>
  </main>

### 导入 CSV 数据文件

**场景描述**：将 `/output` 目录下所有已支持的 CSV 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。CSV 数据文件（后缀名 .csv）请参见 [RFC 4180](http://mirrors.nju.edu.cn/rfc/inline-errata/rfc4180.html) 规范，CSV 数据文件以纯文本形式存储，可通过文本编辑器或者 Excel 等工具直接打开。
...
**场景描述**：将 `/output` 目录下所有已支持的 SQL 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。SQL 数据文件（后缀名 .sql）存储的是 Insert SQL 语句，可通过文本编辑器或者 SQL 编辑器等工具直接打开。
...
**场景描述**：将 `/output` 目录下所有已支持的 POS 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。导入时需要指定控制文件所在的路径。POS 数据文件（默认后缀名 .dat）中的数据是按照固定长度的字节偏移位置所组织的数据，导入时需使用控制文件定义每个字段的固定长度，可通过文本编辑器等工具直接打开。
...
```
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>要求数据库中定义的表名与其对应的控制文件名大小写一致，否则 OBLOADER 无法识别控制文件。控制文件的定义规则请参见 <a href="300.obloader-data-processing/200.obloader-preprocessing-functions.md">预处理函数</a>。</p>
  </main>

### 导入 CUT 数据文件

**场景描述**：将 `/output` 目录下所有已支持的 CUT 数据文件导入到 Schema USERA（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。导入时需要指定控制文件所在的路径，同时使用 `|@|` 作为列分隔字符串。CUT 数据文件（后缀名 .dat）中的数据是以单个字符或者字符串分隔的数据文件，可通过文本编辑器等工具打开。
...
**场景描述**：将 Amazon S3 云存储空间中的所有已支持的 CSV 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
`-f 's3://obloaderdumper/obdumper?region=cn-north-1&access-key=******&secret-key=******'` 选项为存储路径，其组成部分包括：
...
**场景描述**：将 Aliyun OSS 云存储空间中的所有已支持的 CSV 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
`-f 'oss://antsys-oceanbasebackup/backup_obloader_obdumper/obdumper?endpoint=https://cn-hangzhou-alipay-b.oss-cdn.aliyun-inc.com&access-key=******&secret-key=******'` 选项为存储的统一资源定位符，其组成部分包括：
...
**场景描述**：将 Apache Hadoop 云存储空间中的所有已支持的 CSV 数据文件导入到 Schema USERA 中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
`-f 'hdfs://***.*.*.*:9000/chang/parquet?hdfs-site-file=/data/0/zeyang/hdfs-site.xml&core-site-file=/data/0/zeyang/core-site.xml'` 选项为存储的统一资源定位符，其组成部分包括：
...
**场景描述**：支持将 sql、csv、orc、parquet（带列名） 数据文件导入到 OBKV 中。
...
* `--hbase-cf`：指定写入 hbase 时的 ColumnFamily。
* `--hbase-rowkey`：指定写入 hbase 时的 rowkey 列名。
* `--hbase-rowkey-reverse`：指定写入 hbase 时的 rowkey 列名是否需要反转。
...
| 列名 | column1（rowkey） | columnA | columnB | ... |
|------|-------------------|---------|---------|-----|
| 数据行 | v1 | v2 | v3 | ... |
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>其中T存储的是存入当前时间，格式为 负值形式的 `System.currentTimeMillis()`。</p>
</main>
...
* 阅读产品简介，了解导数工具的原理，主要功能，及与其他工具的差异。想要更详细地理解导数工具，请参考官方文档[《OceanBase 导数工具》](https://www.oceanbase.com/docs/oceanbase-dumper-loader-cn)。
