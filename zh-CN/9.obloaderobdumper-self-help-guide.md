...
- 方法 1：[下载 JDK 压缩包](https://www.oracle.com/cn/java/technologies/downloads/)，解压即用。
...
3. 重新进入终端窗口或者使用 `source` 命令使环境变量生效，运行 `jps` 命令确认是否正常运行。
...
重点关注 FGC, FGCT 两项指标，用于描述 Full GC 总次数和总耗时。一般情况下，程序运行期间发生 Full GC 可能是内存比较紧张，用户可以调整 JVM 内存参数 (-Xms/-Xmx)。使用 OpenJDK 1.8 部分的小版本，可能还会出现JVM Crash 或者夯住的现象。

## 查看内存占用

`jmap -dump:format=b,file=./pid.hprof` 命令是 **pid** 将进程内存状态转储到 `./pid.prof`，再使用 MAT 进行分析。
...
上图表示导入数据时，整个导入进程的处理进度。

导入程序的任务划分粒度是文件切分后的总数量，每次成功导入一个子文件，内存中的进度会及时更新。为了体验上显示相对精确的进度信息，通常将文件切分地更小。默认值：64MB，用户无需修改。

### 内核监控

![6](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/421/selfhelp6.png)

上图表示导入数据时，表/分区的分布以及数据库的内存水位。

OceanBase 是分布式数据库，表/分区可能分布在不同的节点上，日志中会记录每个节点的合并状态，内存使用率。

- `Merge Status: true` 表示数据库正在合并，工具停止导入，等待合并结束，工具恢复导入。

- `Mem Used Ratio` 租户内存使用率超过 75%，工具进入限流。

- `Load Mode: SLOW` 表示慢速导入。

- `Mem Used Ratio` 租户内存使用率超过 85%，工具停止导入。

- `Load Mode: PAUSE` 表示停止导入；等待内核转储完毕，内存正常释放，工具才会恢复导入。

- `Load Mode: FAST` 表示快速导入。

性能监控功能支持实时采集和计算导数工具的性能，停止导入时监控指标可能会出现下降的趋势，恢复导入时监控指标会逐渐上升。所以用户使用导数工具过程中，运行速度一会快一会慢。
...
表格中记录的是表名，解析的记录数和导入的记录数以及导入状态。其次记录整个导入过程的总记录数 `Total Count`，以及结束时间 `End Time`。最后记录整个导入的总耗时 `Total Elapsed`，以及进程的退出码。（0 正常退出，非 0：异常退出）
...
关闭控制台输出，即要求所有的日志输出到文件中。

1. 编辑 log4j2.xml 配置文件。

2. 注释第一个 `<AppenderRef ref="ConsoleAppender"/>`。

3. 将第二个 `<AppenderRef ref="ConsoleAppender"/>` 改成 `<AppenderRef ref="InfoRoutingAppender"/>`。