...
关闭控制台输出，即要求所有的日志输出到文件中。编辑 log4j2.xml 配置文件。

1. 将第一个 `<AppenderRef ref="ConsoleAppender"/>` 注释掉。

2. 将第二个 `<AppenderRef ref="ConsoleAppender"/>` 改成 `<AppenderRef ref="InfoRoutingAppender"/>`。