...
| --null-string             | 否    | 指定替代为 `NULL` 的字符。默认值：\\N 。                          |          |          |
| --empty-string            | 否    | 指定替代为空字符（' '）的字符。值默认：\\E 。                          |          |          |
...
| --date-value-format       | 否    |   控制 Date 类型的导出格式。                              |          |          |
| --time-value-format       | 否    |   控制 MySQL 模式下 Time 类型的导出格式。                              |          |          |
| --datetime-value-format   | 否    |  控制 MySQL 模式下 Datetime 类型的导出格式。                               |          |          |
| --timestamp-value-format  | 否    |    控制 MySQL/Oracle 模式下 Timestamp 类型的导出格式。                             |          |          |
| --timestamp-tz-value-format  | 否    |  控制 Oracle 模式下 timestamp with time zone 类型的导出格式。                               |          |          |
| --timestamp-ltz-value-format | 否    |  控制 Oracle 模式下 timestamp with local time zone 类型的导出格式。                             |          |          |
|  --exclude-virtual-columns   | 否    |  标识不导出生成列的数据（默认导出生成列数据）。                               |          |          |
...
* --exclude-column-names **'** ***column_name [, column_name...]*** **'** 
  ...
  用于表示导出数据时排除指定的列。列名不支持模糊匹配。指定多个表时，表名用逗号分隔（,）。导入 Oracle 模式租户的表名默认按照大写处理，导入 MySQL 模式租户的表名默认按照小写处理；例如：OceanBase Oracle 模式下，`--table 'test'` 和 `--table 'TEST'` 均表示 TEST 表；OceanBase MySQL 模式下，`--table 'test'` 和 `--table 'TEST'` 均表示 test 表。如果需要区分大小写，请将表名放入中括号内（[ ]）。例如：`--table '[test]'` 表示 test 表，`--table '[TEST]'` 表示 TEST 表。如果表名指定为星号（*），表示导入所有的表定义或者表数据。
  > **注意**  
  > 仅在排除指定的列后，如果数据文件中的字段顺序与表结构中的字段顺序保持一致时该选项方可生效。
  > 导出的数据文件中，被排除的指定列无数据。
* --skip-header
...
* --null-string ' ***null_replacer_string*** '
  ...
  用于标识将指定字符替代为 NULL。该选项仅与 `--csv` 或者 `--cut` 选项搭配使用。默认值： ***\\N*** 。
<br><br>
* --empty-string ' ***empty_replacer_string*** '

  用于标识将指定字符替代为空字符（' '）。该选项仅与 `--csv` 或者 `--cut` 选项搭配使用。默认值： ***\\E*** 。
<br><br>
