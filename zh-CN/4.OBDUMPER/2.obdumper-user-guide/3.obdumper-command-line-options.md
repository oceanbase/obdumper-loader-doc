...
* --null-string ' ***null_replacer_string*** '
  ...
  用于标识将指定字符串替代为 NULL。该选项仅与 `--csv` 或者 `--cut` 选项搭配使用。默认值： ***\\N*** 。
  > **注意**  
  > 该选项不支持将 `--` 字符替代为 NULL。例如：--null-string ' -- '。

* --empty-string ' ***empty_replacer_string*** '

  用于标识将指定字符串替代为空字符。该选项仅与 `--csv` 或者 `--cut` 选项搭配使用。默认值： ***\\E*** 。
  > **注意**  
  > 该选项不支持将 `--` 字符替代为空字符。例如：--empty-string ' -- '。
