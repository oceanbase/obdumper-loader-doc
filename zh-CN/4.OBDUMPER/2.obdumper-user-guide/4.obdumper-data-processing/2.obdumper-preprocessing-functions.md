...
| TO_TIMESTAMP(char,fmt)                                                   | String   | 对参数值进行日期格式化，若转换失败返回 `NULL`。 参数 `char` 的值可以是列名、常量或函数表达式，参数 `fmt1` 的值是日期解析模板，参数 `fmt2` 的值是日期格式化模板（可选）。默认返回的日期格式： yyyy-MM-dd HH:mm:ss.SSS。                           |
...
| SYSTIMESTAMP                                                             | String   | 对参数不作任何处理，直接返回当前机器的时间戳。格式：yyyy-MM-dd HH:mm:ss.SSS。                                                                                                                                |
| SYSDATE                                                                  | String   | 对参数不作任何处理，直接返回当前机器的日期。格式：yyyy-MM-dd HH:mm:ss。                                                                                                                                     |
