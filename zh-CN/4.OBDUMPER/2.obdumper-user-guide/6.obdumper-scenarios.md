...
**文件类型定义**：ORC 数据文件（后缀名 .orc）是按照列式存储的文件格式，主要用于查询时只读取涉及的列存数据以降低磁盘吞吐量，可通过文本编辑器等工具或者 Excel 打开。

**场景描述**：将 Schema USERA 中所有已支持的对象的数据导出到 `/output` 目录中，数据格式为 ORC 格式。

**示例语句**：

```shell
[admin@localhost]> ./obdumper -h 10.0.0.0 -P 2883 -u test -p ****** --sys-password ****** -c cluster_a -t mysql -D USERA --orc --all -f /output
```
