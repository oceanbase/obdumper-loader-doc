...
A：当前查询所依赖的数据版本已经被回收，用户需要根据查询设置 UNDO 的保留时间。 <br> 例如：`set global undo_retention=xxx`（OceanBase 数据库 V4.0.0-BP1 之前的版本）；`alter system set undo_retention=xxxx;`（OceanBase 数据库 V4.0.0-BP1 之后的版本）。默认单位：秒。
<br><br><br>
