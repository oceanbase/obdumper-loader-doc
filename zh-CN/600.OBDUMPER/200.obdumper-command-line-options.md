...
* --file-name **'** ***file_name*** **'** 
  ...
  | --file-name {*}  | 导出单表多表均可使用。<br>按照表名合并导出的子文件为 n 个文件，n 等于导出表的数量，文件名与表名相同；不会移动 Schema 与 Record 的导出路径。         | 满足以下条件：<ul><li>-f='/home/admin/foo'</li><li>--file-name='\*'</li><li>--table='\*'</li></ul>任务结果：所有记录文件被按表名分组合并到 '/home/admin/foo/data/{database}/TABLE' 路径下；所有 Schema 文件保持原始位置。<main id="notice" type='notice'><h4>注意</h4><p>不建议 <code>--file-name '*'</code> 与 <code>--skip-check-dir</code> 选项搭配使用，因指定 <code>--skip-check-dir</code> 选项会合并该路径下已有的同表名数据文件导致数据重复等问题。</p></main>    |
