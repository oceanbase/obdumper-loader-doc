...
OBDUMPER 通过命令行选项指定导出所需要的信息，更多选项描述和使用示例，请参考 **选项列表** 与 **[使用示例](500.obdumper-scenarios.md)**。
...
| --endpoint                | 否    | 指定 OSS Bucket 所在地域的 Endpoint。 4.1.0 之前的版本中该选项为  --oss-point。                      |      4.1.0    |          |
...
* --table ' ***table_name [,table_name...]*** ' | --table ' **\*** '
  ...
  > **说明**  
  > OBDUMPER 4.1.0 及之后的版本支持导出 OceanBase Oracle 模式下的临时表定义。
...
* --endpoint ' ***oss_endpoint_string*** '
  ...
  > **注意**  
  > 4.1.0 之前的版本中该选项为 `--oss-point`。

* --access-key ' ***access_key_string*** '

  用于标识连接 OSS 服务的访问者。该选项必须与 `--endpoint`, `--secret-key` 搭配使用，用于连接 OSS 云存储服务。
<br><br>
* --secret-key ' ***secret_key_string*** '

  用于标识连接 OSS 服务的密钥串。该选项必须与 `--endpoint`, `--access-key` 搭配使用，用于连接 OSS 云存储服务。
<br><br>
* --bucket-uri ' ***bucket_uri_string*** '

  用于标识 OSS 存储空间的资源位置。该选项必须与 `--endpoint`, `--access-key`, `--secret-key` 搭配使用，表示数据位于 OSS 云存储的指定位置中。该选项与 `-f` 和 `--file-name` 组合使用时遵循以下规律：
  ...
  以导出数据库 'db' 下的表 "tab" 为例，通过几种组合来说明用法：

  |  **-f** |**--file-name**|**--bucket-uri** |OSS 上的 Record 父级路径|
  |----------------------------------|--------------------------------------------------------|-------------------------------------------------------------------------------|-----|
