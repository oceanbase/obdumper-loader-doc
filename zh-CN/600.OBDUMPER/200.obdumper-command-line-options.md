...
* -h ***host_name*** , --host= ***host_name*** 
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>OBDUMPER 4.2.6 及之后的版本支持填入多个 IP 或者域名，以逗号分隔。</p>
  </main>
...
* -P ***port_num*** , --port= ***port_num*** 
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <ul>
    <li>OBDUMPER 4.2.6 及之后的版本支持填入一个或者多个端口，以逗号分隔。</li>
    <li>仅填入一个端口时，表示所有 Host 的端口相同。</li>
    </ul>
  </main>
...
* --public-cloud
  ...
  用于标识将云数据库 OceanBase 部署的 OceanBase 集群中的数据库对象或者表数据导出到文件中。如果命令行指定该选项时则无需指定 `-t`, `-c` 连接选项，同时导出程序默认会打开 `--no-sys` 选项。关于 `--no-sys` 选项请参考对应的选项说明。
<br><br> 
* --no-sys

  用于标识 OceanBase 数据库环境下用户无法提供 sys 租户的密码，将 OceanBase 数据库部署的 OceanBase 集群中的数据库对象或者表数据导出到文件中。该选项与 `--public-cloud` 选项的区别：使用 `--no-sys` 选项时命令行需要指定 `-t` 连接选项，连接 ODP 服务时还需要加上 `-c` 选项。在 OceanBase 4.0.0 及之前的版本中，如果用户未指定 `--public-cloud` 或者 `--no-sys` 选项，OBDUMPER 必须指定 `--sys-user` 和 `--sys-password` 选项。
<br><br>
* --logical-database
...
* -f **'** ***file_path*** **'** , --file-path= **'** ***file_path*** **'** 
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>OBDUMPER 4.2.7 及之后的版本，导出单个文件时支持指定 <code>-f</code> 为具体的文件名。<ul><li> <code>-f</code> 指定的文件名包含后缀时，会覆盖 <code>--file-suffix</code> 选项；文件名不包含后缀时，则会应用 <code>--file-suffix</code> 及其默认值。</li><li>指定 <code>--query-sql</code> 时，会替换默认的文件名 <code>CUSTOM_SQL</code>。</li><li><code>log</code> 目录会被生成在 <code>-f</code> 指定路径的父目录。</li></ul></p>
  </main>
...
* --escape-character ' ***escape_char*** '
  ...
  用于指定转义字符。该选项仅支持单字符，仅与 `--csv` 或者 `--cut` 选项搭配使用。默认值：反斜杠。
   ...
   <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>如需指定该选项为特殊（不可见）字符时，请使用 HEX 表示。</p>
   </main> 
