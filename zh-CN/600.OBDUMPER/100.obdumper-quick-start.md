...
```shell
./obdumper -h 'IP地址' -P'端口' -u'用户@租户名#集群名' -t'租户' -p'密码' -D'库名' --table '表名' --csv -f '文件路径' --sys-password '系统租户密码' --skip-check-dir
```
...
**场景描述**：将 Schema USERA 中所有已支持的对象定义语句导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
```
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><code>--sys-user</code> 选项用于连接 sys 租户下具备特定权限的用户。导出时如果未指定 <code>--sys-user</code> 选项，默认指定的是 <code>--sys-user root</code>。OceanBase 数据库 V4.0.0 及之后的版本无需指定该选项，详情请参见 <a href="200.obdumper-command-line-options.md">OBDUMPER 命令行选项</a>。</p>
  </main>
...
**场景描述**：将 Schema USERA 中所有表中的数据按照 CSV 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。CSV 数据文件（后缀名 .csv）请参考 [RFC 4180](http://mirrors.nju.edu.cn/rfc/inline-errata/rfc4180.html) 规范，CSV 数据文件以纯文本形式存储，可通过文本编辑器或者 Excel 等工具直接打开。
...
**场景描述**：将 Schema USERA 中所有表中的数据按照 SQL 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。SQL 数据文件（后缀名 .sql）存储的是 Insert SQL 语句，可通过文本编辑器或者 SQL 编辑器等工具直接打开。
...
**场景描述**：将 Schema USERA 中所有表中的数据按照 CUT 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。同时指定导出数据的列分隔字符串为 `|@|`。CUT 数据文件（后缀名 .dat）中的数据是以单个字符或者字符串分隔，可通过文本编辑器等工具直接打开。
...
`-f 's3://obloaderdumper/obdumper?region=cn-north-1&access-key=******&secret-key=******'` 选项为存储的统一资源定位符，其组成部分包括：
...
**场景描述**：将 Schema USERA 中 table 表的数据按照 CSV 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。同时指定导出的文件名称为 filetest。
...
**场景描述**：将 Schema USERA 中 table 表的数据按照 CSV 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。同时指定控制文件的路径为 `/output`，为导出的数据进行预处理。
...
```
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>要求数据库中定义的表名与其对应的控制文件名大小写一致，否则 OBDUMPER 无法识别控制文件。控制文件的定义规则请参见 <a href="../600.OBDUMPER/300.obdumper-data-processing/200.obdumper-preprocessing-functions.md">预处理函数</a>。</p>
  </main>

### 导出表中指定列的数据

**场景描述**：将 Schema USERA 中 table 表的数据按照 CSV 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。同时指定 `--exclude-column-names` 选项忽略表中不需要被导出的列。
...
**场景描述**：将 `--query-sql` 选项指定的查询语句执行的结果集按照 CSV 格式导出到 `/output` 目录中（OceanBase 数据库 V4.0.0 之前的版本要求提供 sys 租户的密码）。
...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>在云数据库 OceanBase 或者 OceanBase 数据库环境中无法提供 sys 租户密码时，导出数据库对象定义功能可能存在缺陷。例如：MySQL 兼容模式无法导出序列定义；OceanBase 数据库 V2.2.70 之前的版本，无法导出表组定义和唯一索引的分区信息；OceanBase 数据库 Oracle 兼容模式 V2.2.30 及之前的版本，无法导出索引定义；OceanBase 数据库 Oracle 兼容模式 V2.2.70 (含) ~ V4.0.0.0 的版本，无法导出分区表的唯一索引定义。</p>
  </main>
...
* 阅读产品简介，了解导数工具的原理、主要功能，以及与其他工具的差异。导数工具的更多详情请参见官方文档 [《OceanBase 导数工具》](https://www.oceanbase.com/docs/oceanbase-dumper-loader-cn)。
