...
```
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>在云数据库 OceanBase 或者 OceanBase 数据库环境中无法提供 sys 租户密码时，导出数据库对象定义功能可能存在缺陷。例如：MySQL 兼容模式无法导出序列定义；OceanBase 数据库 V2.2.70 之前的版本，无法导出表组定义和唯一索引的分区信息；OceanBas 数据库 Oracle 兼容模式 V2.2.30 及之前的版本，无法导出索引定义；OceanBas 数据库 Oracle 兼容模式 V2.2.70 (含) ~ V4.0.0.0 的版本，无法导出分区表的唯一索引定义。</p>
  </main>
