...
OBDUMPER 可通过预定义的控制文件对导出的数据进行预处理。有关控制文件的生效规则、列选择、POS 定长文件、常用场景和问题排查，请参见[控制文件使用指南](../../650.control-file-guide.md)。
...
* 列名：数据库表结构中的字段名称。MySQL 模式下，OBDUMPER 始终不区分控制文件列名的大小写。Oracle 模式下，未使用中括号或反引号的列名按大写归一化后匹配；使用中括号或反引号时保留大小写，必须与数据库列名精确匹配。例如：`[c1]` 表示 c1 列，`[C1]` 表示 C1 列。
...
* 预处理函数：在控制文件中为指定的列配置[预处理函数](../300.obdumper-data-processing/200.obdumper-preprocessing-functions.md)，以对导出的数据进行预处理。

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>控制文件的命名规范为 <strong>&lt;表名&gt;.ctrl</strong>。</p>
    <p>当 <code>--ctl-path</code> 指向目录时，OBDUMPER 会递归查找该目录及其子目录下的 <code>.ctrl</code> 文件。</p>
  </main>
...
(
  ...
  c12 "lpad(c12,5,'x')",                                                                          -- c12 列的值左侧追加 5 个字符长度字符串 'x'
  c13 "rpad(c13,5,'x')",                                                                          -- c13 列的值右侧追加 5 个字符长度字符串 'x'
  ...
  c20 "lpadb(c20,5,'x')",                                                                         -- c20 列的值左侧追加 5 个字节长度（单）字符 'x'
  c21 "rpadb(c21,5,'x')",                                                                         -- c21 列的值右侧追加 5 个字节长度（单）字符 'x'
...
```
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>详情请参见 <strong>预处理函数</strong> 中的函数列表。</p>
  </main>
