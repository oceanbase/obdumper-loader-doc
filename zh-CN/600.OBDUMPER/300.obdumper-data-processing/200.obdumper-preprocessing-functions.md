...
| TO_TIMESTAMP(char,fmt1[,fmt2])                                                  | String   | 对参数值进行日期格式化，如果转换失败返回 `NULL`。参数 `char` 的值可以是列名、常量或函数表达式，参数 `fmt1` 的值是日期解析模板，可选参数 `fmt2` 的值是日期格式化模板。<br> 参数 `fmt2` 返回值的默认格式：yyyy-MM-dd HH:mm:ss.SSS。                |
...
| TMSFMT(char,fmt1,default,fmt2)                                          | String   | 对参数进行日期格式验证，如果验证失败，则返回默认的日期格式。<br> 示例：`TMSFMT(c1,'yyyyMMddHHmmssSSS','2021-03-10 00:00:00.000','yyyy-MM-dd HH:mm:ss.SSS')`。                                            |
...
|TIMESTAMP_ADD (dateAndTime, addInt, datetimeFieldString, pattern: yyyy-MM-dd HH:mm:ss)|String|增加/减少时间戳（TIMESTAMP）类型的值。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、WEEK、DAY、HOUR、MINUTE 和 SECOND。</li><li>`addInt` 表示增减值，设为负值表示减。</li><li>`pattern` 为可选参数，表示原始时间的格式。默认值：yyyy-MM-dd HH:mm:ss。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`TIMESTAMP_ADD(c1, '1', 'MONTH', 'yyyy-MM-dd HH:mm:ss')`。<br>不填写 `pattern` 的示例：`TIMESTAMP_ADD(c1, '1', 'MONTH')`，表示 c1 列的值增加一整月。<br>`TIMESTAMP_ADD(c1, '-2', 'WEEK')`，表示 c1 列的值减少两周。|
...
|DATE_TRUNC (date, datetimeFieldString, pattern: yyyy-MM-dd)|String|日期（DATE）类型的值截断到指定的字段。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、WEEK 和 DAY。</li><li>`pattern` 为可选参数，表示原始日期的格式。默认值：yyyy-MM-dd。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`DATE_TRUNC(c1, 'MONTH', 'yyyy-MM-dd')`。<br>不填写 `pattern` 的示例：`DATE_TRUNC(c1, 'MONTH')`，表示 c1 列的值截断到月。|
|TIMESTAMP_TRUNC (dateAndTime, datetimeFieldString,pattern: yyyy-MM-dd HH:mm:ss)|String|时间戳（TIMESTAMP）类型的值截断到指定的字段。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、WEEK、DAY、HOUR、MINUTE 和 SECOND。</li><li>`pattern` 为可选参数，表示原始日期的格式。默认值：yyyy-MM-dd HH:mm:ss。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`TIMESTAMP_TRUNC(c1, 'HOUR', 'yyyy-MM-dd HH:mm:ss')`。<br>不填写 `pattern` 的示例：`TIMESTAMP_TRUNC(c1, 'HOUR')`，表示 c1 列的值截断到小时。|
