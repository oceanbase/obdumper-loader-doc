...
| SUBSTR(char,position[,length ])                                       | String   | 对参数值根据起始位置与长度进行截断。`position` 从 1 开始：正数从字符串开头计数，`0` 按 `1` 处理，负数从字符串末尾倒数。参数 `char` 的值可以是列名、常量或函数表达式，参数 `position` 和 `length` 的值是数值型常量。<br>示例：`SUBSTR('abc',0,3)`。                                |
...
|MINUTE (dateAndTime, pattern: yyyy-MM-dd HH:mm:ss)|String|返回时间戳（TIMESTAMP）类型的值对应的分钟数（1-59）。函数关键字不区分字母大小写。<br>示例：`MINUTE (c1)`，c1 列的值返回分钟数。|
