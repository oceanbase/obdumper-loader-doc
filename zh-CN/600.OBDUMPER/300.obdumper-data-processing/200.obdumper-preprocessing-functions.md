...
|NCHR(string)|String|将字符串表示的数字转换成 NVARCHAR2 类型。<ul><li>区别于 CHR，NCHR 可以处理大于 256 的参数。示例：<code>NCHR('257')</code> 将返回 <code>ā</code>。</li><li>如果任意一个参数为 NULL， NCHR() 将返回 NULL。</li><li>参数值可以是列名或常量。当参数值为<strong>常量</strong>时，请使用<strong>单引号</strong>。<br>示例：<code>NCHR('13')</code>, <code>NCHR(c1)</code>。</li></ul>|
