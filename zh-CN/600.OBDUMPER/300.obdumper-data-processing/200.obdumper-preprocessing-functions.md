...
|TRUNC(number[, decimal])|String|对数字进行截断。<br>`number` 表示被截断的数字；`decimal` 表示截断的小数位。示例：`TRUNC(22.29, 1)`，返回 `22.2`。<ul><li>如果省略 `decimal`，则 `number` 将被截断至 0 位。示例：`TRUNC(22.29)`，返回 `22`。</li><li>`decimal` 可以为负数，表示截断 `number` 小数点左侧的数字（使其为零）。示例：`TRUNC(22.29, -1)`，返回 `20`。</li></ul>此函数将任何数值数据类型或者任何可隐式转换为数值数据类型的非数值数据类型作为参数。|
