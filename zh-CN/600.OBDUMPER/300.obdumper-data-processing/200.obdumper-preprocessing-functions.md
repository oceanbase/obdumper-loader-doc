...
|---|---|
|machine-translation||
|slug|obdumper-preprocessing-functions|
...
| LTRIM(char[,set])                                                     | String   | 对参数值从左到右进行匹配并截断操作。参数 `char` 的值可以是列名、常量或函数表达式。参数 `set` 的值是常量。如果省略参数 `set`，则默认按照空格进行匹配。<br> 示例：`LTRIM(' abc ')`。                                        |
| RTRIM(char[,set])                                                     | String   | 对参数值从右到左进行匹配并截断操作。参数 `char` 的值可以是列名、常量或函数表达式，参数 `set` 的值是常量。如果省略参数 `set`，则默认按照空格进行匹配。<br> 示例：`RTRIM(' abc ')`。                                        |
| SUBSTR(char,position[,length ])                                       | String   | 对参数值根据起始位置与长度进行截断。`position` 从 1 开始：正数从字符串开头计数，`0` 按 `1` 处理，负数从字符串末尾倒数。参数 `char` 的值可以是列名、常量或函数表达式，参数 `position` 和 `length` 的值是数值型常量。<br>示例：`SUBSTR('abcdef',2,3)` 返回 `bcd`；`SUBSTR('abcdef',-3,2)` 返回 `de`；`SUBSTR('abcdef',4)` 返回 `def`；`SUBSTR('abc',0,3)` 返回 `abc`。                                |
...
| NANVL(char, default)                                                    | String   | 对参数值进行数值合法验证，如果验证失败，则返回默认值。参数 `char` 的值可以是列名、常量或函数表达式，参数 `default` 的值是常量。<br> 示例：`NANVL('1','1')`。                                                    |
...
| NVL(char, default)                                                      | String   | 对参数值进行判空，如果为空，则返回默认值。参数 `char` 的值可以是列名、常量或函数表达式，参数 `default` 的值是常量。<br> 示例：`NVL('a','--')`。                                                           |
| TO_TIMESTAMP(char,fmt1[,fmt2])                                                  | String   | 对参数值进行日期格式化。仅当 `char` 为 `NULL` 时返回 `NULL`；非空值转换失败时抛出异常。参数 `char` 的值可以是列名、常量或函数表达式，参数 `fmt1` 的值是日期解析模板，可选参数 `fmt2` 的值是日期格式化模板。<br>参数 `fmt2` 的默认值：yyyy-MM-dd HH:mm:ss.SSS。                |
| LENGTH(char)                                                            | String   | 对参数值进行长度计算。参数 `char` 的值可以是列名、常量或函数表达式。                                                                                                                           |
| LPAD(char,length[,pad_string])                                        | String   | 对参数值从左侧追加指定长度的字符，如果指定的长度小于参数长度则截断。参数 `char` 的值可以是列名、常量或函数表达式，参数 `length` 和 `pad_string` 的值是常量，且参数 `pad_string` 的值要求传入单字节字符。<br> 示例：`LPAD('a',1,'x')`。 |
...
| TMSFMT(char,fmt1,default[,fmt2])                                          | String   | 对参数进行日期格式验证。如果转换失败，返回第 3 个参数 `default` 指定的替换值。第 4 个参数 `fmt2` 可选，表示目标格式，默认值为 `yyyy-MM-dd HH:mm:ss.SSS`。<br>示例：`TMSFMT(c1,'yyyyMMddHHmmssSSS','2021-03-10 00:00:00.000','yyyy-MM-dd HH:mm:ss.SSS')`。                                            |
...
| SEQUENCE(initial[,increment])                                                   | String   | 对指定的列生成序列值。`initial` 表示初始值，必填；`increment` 表示步长，可选，默认值为 `1`。<br>示例：`SEQUENCE(100)` 生成初始值为 100、步长为 1 的序列。                                                                                        |
...
|HEXTORAW(string)|String|将十六进制表示的字符串还原成原始字符串。<br>参数值可以为列名或常量。当参数值为<strong>常量</strong>时，请使用<strong>单引号</strong>。<br>示例：`HEXTORAW('414243')`, `HEXTORAW(c1)`。<main id="notice" type='explain'><h4>说明</h4><p>OBDUMPER 仅支持处理不包含 <code>0x</code> 或仅以 <code>0x</code> 为前缀的十六进制数据，请通过嵌套  <code>replace</code> 函数去除包含 <code>0x</code> 的数据（非前缀）。示例：<code>lang=java (c1 "hextoraw(replace(c1, '0x', ''))");</code></p></main>|
...
|TIMESTAMP_ADD (dateAndTime, addInt, datetimeFieldString, pattern: yyyy-MM-dd HH:mm:ss)|String|增加/减少时间戳（TIMESTAMP）类型的值。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、WEEK、DAY、HOUR、MINUTE、SECOND 和 MILLISECOND。</li><li>`addInt` 表示增减值，设为负值表示减。</li><li>`pattern` 为可选参数，表示原始时间的格式。默认值：yyyy-MM-dd HH:mm:ss。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`TIMESTAMP_ADD(c1, '1', 'MONTH', 'yyyy-MM-dd HH:mm:ss')`。<br>不填写 `pattern` 的示例：`TIMESTAMP_ADD(c1, '1', 'MONTH')`，表示 c1 列的值增加一整月。<br>`TIMESTAMP_ADD(c1, '-2', 'WEEK')`，表示 c1 列的值减少两周。|
|DATE_ADD (date, addInt, datetimeFieldString, pattern: yyyy-MM-dd)|String|增加/减少日期（DATE）类型的值。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、WEEK 和 DAY。</li><li>`addInt` 表示增减值，设为负值表示减。</li><li>`pattern` 为可选参数，表示原始时间的格式。默认值：yyyy-MM-dd。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`DATE_ADD(c1, '-1', 'YEAR', 'yyyy-MM-dd')`。<br>不填写 `pattern` 的示例：`DATE_ADD(c1, '-1', 'YEAR')`，表示 c1 列的值减少一整年。|
|DATE_TRUNC (date, datetimeFieldString, pattern: yyyy-MM-dd)|String|日期（DATE）类型的值截断到指定的字段。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH 和 DAY。</li><li>`pattern` 为可选参数，表示原始日期的格式。默认值：yyyy-MM-dd。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`DATE_TRUNC(c1, 'MONTH', 'yyyy-MM-dd')`。<br>不填写 `pattern` 的示例：`DATE_TRUNC(c1, 'MONTH')`，表示 c1 列的值截断到月。|
|TIMESTAMP_TRUNC (dateAndTime, datetimeFieldString,pattern: yyyy-MM-dd HH:mm:ss)|String|时间戳（TIMESTAMP）类型的值截断到指定的字段。<ul><li>`datetimeFieldString` 表示单位。可选值包括 YEAR、MONTH、DAY、HOUR、MINUTE 和 SECOND。</li><li>`pattern` 为可选参数，表示原始日期的格式。默认值：yyyy-MM-dd HH:mm:ss。<br>如果 `pattern` 的值和默认值相同，可填可不填。如果和默认值不相同，则该参数的值为必填。</li></ul>填写 `pattern` 的示例：`TIMESTAMP_TRUNC(c1, 'HOUR', 'yyyy-MM-dd HH:mm:ss')`。<br>不填写 `pattern` 的示例：`TIMESTAMP_TRUNC(c1, 'HOUR')`，表示 c1 列的值截断到小时。|
...
|DAY_OF_WEEK (dateAndTime, pattern: yyyy-MM-dd HH:mm:ss)|String|返回时间戳（TIMESTAMP）类型的值对应的星期，返回值为 `MONDAY`～`SUNDAY` 的大写英文字符串。<br>示例：`DAY_OF_WEEK (c1)`，c1 列的值返回对应的星期。|
...
|MINUTE (dateAndTime, pattern: yyyy-MM-dd HH:mm:ss)|String|返回时间戳（TIMESTAMP）类型的值对应的分钟数（0-59）。<br>示例：`MINUTE (c1)`，c1 列的值返回分钟数。|
|MONTH (dateAndTime, pattern: yyyy-MM-dd HH:mm:ss)|String|返回时间戳（TIMESTAMP）类型的值对应的月份，返回值为 `JANUARY`～`DECEMBER` 的大写英文字符串。<br>示例：`MONTH (c1)`，c1 列的值返回对应的月份。|
...
|sm3_digest (plaintext [,encoding])| String |使用 SM3 算法对数据进行哈希，并返回提取的摘要。函数参数：<ul><li>plaintext：表示原始文本</li><li>encoding：表示生成摘要的编码类型。支持 Base64 与 HEX，默认值为 HEX。</li></ul> <br>示例：c1 "sm3_digest(c1)" <br>c2 "sm3_digest(c2, 'BASE64')"|

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OBDUMPER V3.x 及之后版本不再支持 groovy 动态函数。</p>
  </main>
