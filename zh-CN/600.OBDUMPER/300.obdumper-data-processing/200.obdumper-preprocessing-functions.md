...
|ASCII(char)|String|将字符转换成十进制 ASCII 码。<br>如果传入多个字符，其仅转换第一个字符。例如：`ASCII('A')` 与 `ASCII('abc')` 均返回 `'65'`。<br>参数值可以为列名或常量。当参数值是<strong>常量</strong>时，请使用<strong>单引号</strong>。<br>示例：`ASCII('A')`、`ASCII(c1)`。|
