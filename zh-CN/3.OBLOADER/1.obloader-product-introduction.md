...
* 使用限制模式（`--public-cloud`）从公共云部署的 OceanBase 集群导入数据时无需指定 `-t`, `-c` 连接选项；使用限制模式（`--no-sys`）从私有云部署的 OceanBase 集群导入数据时需要指定 `-t` 连接选项，连接 ODP 服务时还需要加上 `-c` 选项。如果用户未使用限制模式，OBDUMPER 必须指定 `--sys-user` 和 `--sys-password` 选项。
