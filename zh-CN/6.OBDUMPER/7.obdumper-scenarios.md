...
**场景描述**： 将 Schema USERA 中所有表中的数据按照 CSV 格式导出到 S3 云存储空间中。

**示例语句**：

```shell
$./obdumper -h xx.x.x.x -P 2883 -u test -p ****** --sys-user **u*** --sys-password ****** -c cluster_a -t mysql -D USERA --csv --table '*' --storage-uri   
```
