...
|   --fetch-size               | 否    |     标识 Oracle 模式下 JDBC 每次从数据库游标中检索到的行数。                     |       4.2.0   |          |
...
* --storage-uri ' ***storage_uri_string*** '
  ...
  |  **参数** |**说明**|
  |----------------------------------|--------------------------------------------------------|
  |  scheme | 非必选项。支持选择存储类型（OSS/S3）。<br> 如果指定的 Scheme 非当前支持的远端存储（OSS/S3），则会报错。|
...
* --fetch-size ***int_num***
  ...
  用于标识 Oracle 模式下每次从数据库游标中提取的行数。具体配置请参考 [Oracle Row Fetch Size](https://docs.oracle.com/en/database/oracle/oracle-database/19/jjdbc/resultset.html#GUID-BD27B43E-525F-44C4-973F-18D2BB8BA007)。

  <main id="notice" type='explain'>
  <h4>说明</h4>
  <p>此选项需要 JDBC 驱动程序支持。</p>
  </main>
