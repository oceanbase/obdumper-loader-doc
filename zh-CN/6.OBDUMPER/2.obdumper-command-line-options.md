...
|   --upload-behavoir               | 否    |     标识导出数据到云存储场景时的上传模式。                     |       4.2.0   |          |
...
* --fetch-size ' ***int_num*** '
  ...
  用于标识 Oracle 模式下每次从数据库游标中提取的行数。具体配置请参考 [OceanBase FetchSize](https://www.oceanbase.com/docs/common-oceanbase-connector-j-cn-10000000001652365)。
...
* --upload-behavoir ' ***upload_behavoir_string*** '
  ...
  用于标识导出数据到云存储场景时的滚动上传模式。该选项支持 `FAST` 和 `COMPLETE` 两种上传模式。`FAST`：文件导出到本地后即时上传；`COMPLETE`：导出所有文件后再批量上传，适用于需要合并与移动子文件的场景。
<br><br>
* --storage-uri ' ***storage_uri_string*** '

  用于标识云存储的统一资源定位符。OBDUMPER 4.2.0 及之后的版本支持将数据导出到 OSS 存储空间和 Amazon S3 存储空间。`--storage-uri` 选项为定义的标准存储串，通过配置标准存储串导出数据到云存储中。

  ```shell
  [scheme://bucket]path[?parameters]
  ...
  |  **参数** |**说明**|
  |----------------------------------|--------------------------------------------------------|
  |  scheme | 非必选项。支持选择存储类型（[OSS](https://help.aliyun.com/document_detail/31817.html?spm=5176.8465980.help.dexternal.4e701450fT87ve)/S3）。<br> 如果指定的 Scheme 非当前支持的远端存储（OSS/S3），则会报错。|
  | bucket |必选项。指存储空间的名称。<br>导出数据到云存储空间时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。 |
  |  path  | 必选项。指存储空间的资源路径。路径**必须**以 `/` 开头。 |
  |  parameters  |  非必选项。指参数列表。<br>parameters 可为单个 key 或者 key-value。 |

  示例如下：
  |  **参数** |**说明**|
  |----------------------------------|--------------------------------------------------------|
  | 数据导出到 S3 | `--storage-uri 's3://bucket/path?region={region}&access-key={accessKey}&secret-key={secretKey}'` <br> 其中，<ul><li> `s3`：scheme 为 s3。</li><li>`bucket`：存储空间的名称。</li><li>`path`：导出到 S3 存储空间的资源路径。</li><li>`?region={region}&access-key={accessKey}&secret-key={secretKey}`：region、access-key、secret-key 参数值。</li></ul>|

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <ul>
    <li>当且仅当存在 scheme 时，才会同时存在 bucket。</li>
    <li>bucket 选项与 `-f` 和 `--file-name` 组合使用时遵循以下规律：<ul><li>-f 与 bucket 为必选项。</li><li>无论 --file-name 选项是否存在，-f 与上传后文件在云存储空间的 key 不再关联。</li><li>当存在 --file-name 选项且指定不为路径时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当不存在 --file-name 选项时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当上传本地数据文件到云存储空间时，Object key 有以下几种情况：<br> <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/410/410%20bucket%20uri.png" width = "560" /><br>以导出数据库 'db' 下的表 "tab" 为例，通过几种组合来说明用法：<br><img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/420/obdumper%20f%20bucket%20uri.png" width = "560" /></li></ul></li>
    <li>支持的 parameters 包括：<ul><li>key: endpoint<br>value: endpoint，必选项，用于指定 OSS Bucket 所在地域的 Endpoint</li><li>key: region<br>value：[region](https://docs.aws.amazon.com/zh_cn/general/latest/gr/rande.html#s3_region)，必选项，S3 </li><li>key：storage-class<br>value：[storage-class](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/userguide/storage-class-intro.html)</li><li>key: access-key<br>value: Access Key，必选项，用于指定云存储的访问账号</li><li>key: secret-key<br>value: Secret Key，必选项，用于指定云存储的访问密钥</li></ul></li>
    </ul>
  </main>

<p><code>REPLACE INTO</code> 请参见 <a href="../../3.develop/4.replace-data-of-mysql-mode.md">替换数据</a>。</p>
* --date-value-format
