...
|   --with-trim               | 否    |     标识导出 CSV 与 CUT 格式文件时去除列中空格。                     |       4.2.0   |          |
...
|   --fetch-size               | 否    |     标识 Oracle 模式下每次从数据库游标中提取的行数。                     |       4.2.0   |          |
...
|   --upload-behavoir               | 否    |     标识导出数据到云存储场景时的滚动上传模式。                     |       4.2.0   |          |
...
* --fetch-size ' ***int_num*** '
  ...
  用于标识 Oracle 模式下每次从数据库游标中提取的行数。具体配置请参考 [OceanBase Fetch Size](https://www.oceanbase.com/docs/common-oceanbase-connector-j-cn-10000000001652365)。

  <main id="notice" type='explain'>
  <h4>说明</h4>
  <p>此选项需要安装 [OceanBase JDBC](https://www.oceanbase.com/docs/common-oceanbase-connector-j-cn-10000000001652341) 驱动程序。</p>
  </main>
* --upload-behavoir ' ***upload_behavoir_string*** '

  用于标识导出数据到云存储场景时的滚动上传模式。该选项支持 `FAST` 和 `COMPLETE` 两种上传模式。`FAST` 支持将导出的数据分批滚动式上传；`COMPLETE` 支持将导出的数据整体批量上传。
<br><br>
* --storage-uri ' ***storage_uri_string*** '
  ...
  |  **参数** |**说明**|
  |----------------------------------|--------------------------------------------------------|
  |  scheme | 非必选项。支持选择存储类型（[OSS](https://help.aliyun.com/document_detail/31817.html?spm=5176.8465980.help.dexternal.4e701450fT87ve)/S3）。<br> 如果指定的 Scheme 非当前支持的远端存储（OSS/S3），则会报错。<ul><li>[OSS](https://help.aliyun.com/document_detail/31883.html?spm=a2c4g.31882.0.0.423815bd5xSw6l)：仅支持虚拟托管访问方式，具体请参考 [使用 OSS](https://help.aliyun.com/document_detail/31883.html?spm=a2c4g.31882.0.0.423815bd5xSw6l)。</li><li>S3 (Amazon S3)：支持路径（Path）请求风格和虚拟托管（Virtual Hosted）请求风格。</li></ul>|
  | bucket |必选项。指存储空间的名称。<br>导出数据到云存储空间时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。<br>该选项与 `-f` 和 `--file-name` 组合使用时遵循以下规律：<ul><li>-f 与 bucket 为必选项。</li><li>无论 --file-name 选项是否存在，-f 与上传后文件在云存储空间的 key 不再关联。</li><li>当存在 --file-name 选项且指定不为路径时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当不存在 --file-name 选项时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当上传本地数据文件到云存储空间时，Object key 有以下几种情况：<br> <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/410/410%20bucket%20uri.png" width = "560" /><br>以导出数据库 'db' 下的表 "tab" 为例，通过几种组合来说明用法：<br><img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/420/obdumper%20f%20bucket%20uri.png" width = "560" /></li></ul></li></ul><blockquote>注意<br>当且仅当存在 scheme 时，才会存在 container。</blockquote> |
  |  path  | 必选项。指存储空间的资源路径。路径**必须**以 `/` 开头。 |
  |  parameters  |  非必选项。指参数列表。<br>其中，parameters 可为单个 key 或者 key-value，例如 Access Key 和 Secret Key。OBLOADER 4.2.0 支持的参数包括：<ul><li>key: endpoint<br>value: endpoint<br>用于指定 Bucket 所在地域的 Endpoint。</li><li>key: region<br>value：[region](https://docs.aws.amazon.com/zh_cn/general/latest/gr/rande.html#s3_region)</li><li>key：storage-class<br>value：[storage-class](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/userguide/storage-class-intro.html)</li><li>key: access-key<br>value: Access Key<br>用于指定云存储的访问账号。</li><li>key: secret-key<br>value: Secret Key<br>用于指定云存储的访问密钥。</li></ul><blockquote>**注意**<ul><li>OSS 必须的参数包含 endpoint, access-key, secret-key。</li><li>S3 必须的选项包含 region, access-key, secret-key</li></ul></blockquote>  |
