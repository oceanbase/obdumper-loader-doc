...
* --storage-uri ' ***storage_uri_string*** '
  ...
  <ul><li> <code>s3</code>：scheme 为 s3。</li><li><code>bucket</code>：存储空间的名称。</li><li><code>path</code>：导出到 S3 存储空间的资源路径。</li><li><code>?region={region}&access-key={accessKey}&secret-key={secretKey}</code>：指定 region, access-key, secret-key 参数值。</li></ul>
