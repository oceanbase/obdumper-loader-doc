...
* --cut
  ...
  > 以 Delimited Text 文件格式导出时，如果字段内容中出现指定的字段分隔符，会对字段中的分隔符字符进行转义。例如：字段内容为 `abc|def`，分隔符为 `|`，则会导出字段内容为 `abc\|def`。  
  > 以 Delimited Text 文件格式导入时如果行尾没有字段分隔符（或分隔串），请勿使用 `--trail-delimiter` 选项，否则会导致程序运行错误。
...
* --no-sys
  ...
  用于标识私有云环境下无 sys 租户密码的使用场景。与用于公共云的 `--public-cloud` 选项场景进行区分。
<br><br>
* --logical-database
