...
* --storage-uri ' ***storage_uri_string*** '
  ...
  | host |指定存储空间的名称。<br>导出数据到 OSS 时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。 |
  ...
  ```shell
  --storage-uri 's3://host/path?region={region}&access-key={accessKey}&secret-key={secretKey}'
  ```

  <ul><li> <code>s3</code>：scheme 为 s3。</li><li><code>host</code>：存储空间的名称。</li><li><code>path</code>：导出到 S3 存储空间的资源路径。</li><li><code>?region={region}&access-key={accessKey}&secret-key={secretKey}</code>：指定 region, access-key, secret-key 参数值。</li></ul>
