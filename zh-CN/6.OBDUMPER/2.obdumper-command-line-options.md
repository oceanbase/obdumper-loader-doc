...
* --storage-uri ' ***storage_uri_string*** '
  ...
  其中，<ul><li> `s3`：scheme 为 s3。</li><li>`bucket`：存储空间的名称。</li><li>`path`：导出到 S3 存储空间的资源路径。</li><li>`?region={region}&access-key={accessKey}&secret-key={secretKey}`：region、access-key、secret-key 参数值。</li></ul>

  <main id="notice" type='notice'>
    <h4>注意</h4>
    <ul>
    <li>当且仅当存在 scheme 时，才会同时存在 bucket。</li>
    <li>bucket 选项与 <code>-f</code> 和 <code>--file-name</code> 组合使用时遵循以下规律：<ul><li><code>-f</code> 与 bucket 为必选项。</li><li>无论 <code>--file-name</code> 选项是否存在，<code>-f</code> 与上传后文件在云存储空间的 key 不再关联。</li><li>当存在 <code>--file-name</code> 选项且指定不为路径时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当不存在 <code>--file-name</code> 选项时，以 bucket 的路径为准，并拼接生成后缀。例如：data/{schema}/table。</li><li>当上传本地数据文件到云存储空间时，Object key 有以下几种情况：<br> <img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/410/410%20bucket%20uri.png" width = "560" /><br>以导出数据库 'db' 下的表 "tab" 为例，通过几种组合来说明用法：<br><img src="https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/420/obdumper%20f%20bucket%20uri.png" width = "560" /></li></ul></li>
