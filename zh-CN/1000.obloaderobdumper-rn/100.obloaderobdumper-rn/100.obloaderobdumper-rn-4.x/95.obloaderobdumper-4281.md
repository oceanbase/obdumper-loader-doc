...
* OceanBas 数据库 Oracle 兼容模式 结构导出的性能优化仅适用于以下场景：

  * OceanBas 数据库 Oracle 兼容模式 V4.0.0 之前的版本， 使用`--no-sys` 进行限制导出。

  * OceanBas 数据库 Oracle 兼容模式 V4.0.0 及之后的版本，使用新增选项`--compact-schema` 进行导出。
...
* 修复使用 OBDUMPER 导出 OceanBas 数据库 Oracle 兼容模式 V4.0.0 之前的版本数据时，分区表分页耗时太长的缺陷。
...
* OceanBas 数据库 MySQL 兼容模式V2.2.30 之前的版本中均不支持导出 PL 对象定义。
