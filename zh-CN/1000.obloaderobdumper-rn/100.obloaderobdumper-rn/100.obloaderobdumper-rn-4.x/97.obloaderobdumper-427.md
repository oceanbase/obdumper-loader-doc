...
* `--file-name` 选项已弃用。您只需通过 `--block-size` 选项设置单个文件大小的阈值为 `0`，表示不限制文件大小，无需合并单表导出的若干子文件。
   ...
   <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>虽然导数工具不会限制文件大小，某些文件系统或者对象存储仍会限制单个文件的大小。例如：OSS 对单个文件追加上传的阈值为 5G、在 ext2/ext3/ext4 文件系统中，单个文件的大小限制为 2TB。如需去除导数工具默认生成的复杂层次目录结构，您可以使用新增的 <code>--no-nested-dir</code> 选项替代之前 <code>--file-name = '*'</code> 的用法。</p>
   </main>
