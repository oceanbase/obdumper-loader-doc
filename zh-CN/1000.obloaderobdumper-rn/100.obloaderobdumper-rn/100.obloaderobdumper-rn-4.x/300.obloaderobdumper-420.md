...
* OBLOADER：
  ...
  * 支持加密命令行敏感信息。目前支持加密的敏感字段包括：OceanBase 业务租户的密码（oceanbase.jdbc.password）、OceanBase sys 租户下的账户密码（oceanbase.jdbc.sys.password）、云存储的访问账号（cloud.storage.access.key）和云存储的访问密钥（cloud.storage.secret.key）。详情请参考 [加密设置](../../../500.OBLOADER/500.obloader-encryption.md)。
