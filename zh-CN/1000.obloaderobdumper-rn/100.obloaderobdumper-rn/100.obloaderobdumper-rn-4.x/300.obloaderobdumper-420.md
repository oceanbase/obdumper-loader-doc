...
* OceanBas 数据库 MySQL 兼容模式 V2.2.30 之前的版本中均不支持导出 PL 对象定义。

* 当指定 `--query-sql` 导出 DATETIME 类型数据时，可能存在格式问题。

* 命令行未指定 sys 租户密码时，OceanBase 数据库 V2.2.70 之前的版本均不支持导出表组定义。

* 命令行未指定 sys 租户密码时，OceanBas 数据库 Oracle 兼容模式 V2.2.50 之前的版本无法导出索引定义。

* 命令行未指定 sys 租户密码时，OceanBas 数据库 Oracle 兼容模式 V2.2.70 之前的版本无法导出唯一索引的分区信息。

* 命令行未指定 sys 租户密码时，OceanBas 数据库 Oracle 兼容模式 V2.2.70 及之后的版本无法导出分区表的唯一索引定义。
