...
本次 1 月发版的导数工具 V4.2.8 旨在新增 SM4 加解密、日期和时间、导出定长格式相关的预处理函数；OBServer V4.x 支持通过 `SHOW CREATE TABLE` 导出表结构定义；支持通过 `--pos` 选项导出定长格式文件；优化 OceanBase Oracle 租户下查询元数据的效率；以及优化若干命令行选项的使用体验。
