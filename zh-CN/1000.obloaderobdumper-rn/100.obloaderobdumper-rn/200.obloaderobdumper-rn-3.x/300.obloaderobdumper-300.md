...
OceanBase 导数工具 V3.0.0 支持下表所列版本的 OceanBase 数据库。
...
* OBLOADER：
  ...
  * OceanBase 数据库 MySQL 兼容模式 V1.4.x RANGE_COLUMNS + KEY 在虚拟路由视图中的元数据有缺陷。
...
* 限制模式会限制部分导入的功能、性能以及稳定性。同时，OceanBase 数据库 V2.2.30 及之后版本才支持服务端限流的能力，因此在使用限制模式时通过以下命令调整服务端限流：
