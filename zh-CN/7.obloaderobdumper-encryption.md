...
1. 使用 Java 编写一个解密类的文件。类的定义如下：
   ...
   - 拥有非静态方法：`public String decrypt(String arg)`，包括形参 encryptRaw 为加密后的文本和返回值为解密后的文本。
...
1. 新建一个 Java 项目。下图中的项目结构中，`CustomDecryptor.java` 是需要编写的自定义解密类。

   ![java](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/obloaderobdumper/420/encrypt.png)
...
3. 打包成 JAR 并将其放置在 `{ob-loader-dumper}/lib/` 路径下，完成设置解密器。
