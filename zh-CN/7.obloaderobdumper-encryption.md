...
1. 安装 [OpenSSL](https://www.openssl.org/source/?spm=a2c4g.42216.0.0.7097553fHQEFiE) 并配置环境变量。
   ...
   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>使用 secure-gen 加密时，需要安装 OpenSSL 工具包。（查看 secure-gen 帮助）</p>
   </main>
...
1. 使用 Java 编写一个解密类的文件。类的定义如下：
   ...
   - 必须定义非静态方法：`public String decrypt(String arg)`。

2. 将解密类编译成 JAR 包并放置在 `{ob-loader-dumper}/lib/` 路径中。

3. 在 `{ob-loader-dumper}/conf/decrypt.properties` 配置相应的内容。

   例如：编写一个 `CustomDecryptor` 类且使用 Base64 算法进行编解码
   ...
   1. 新建一个项目，定义 `CustomDecryptor.java`。
       ...
       public class CustomDecryptor {
          ...
          */
           public String decrypt(String arg) throws Exception {
                ...
                return new String(decoder.decodeBuffer(arg), StandardCharsets.UTF_8);
            }
  
        ```

   2. 将已编译的 JAR 包放置在 `{ob-loader-dumper}/lib/` 路径下。


   3. 加密类设置完成后，可以通过对应的加密算法加密敏感信息。本例使用 Base64 编码：
