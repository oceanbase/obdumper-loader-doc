...
* 与日期时间相关的预处理函数（`[SYSTIMESTAMP[](https://help.aliyun.com/document_detail/212816.html)](https://help.aliyun.com/document_detail/212816.html)`、`TMSFMT()` 和 `TO_TIMESTAMP()` 等）只能精确到毫秒。此外，要求工具运行的服务器时钟与数据库服务器时钟相同。
...
|DB_SEQUENCE(string sequence)|String|指定数据库中存在的序列进行导入。<br> 示例：`DB_SEQUENCE(seq01)`，指定序列值为 seq01。<blockquote>**说明**</br>OBLOADER 4.1.0 及之后的版本支持该功能。</blockquote>|
