...
* --storage-uri ' ***storage_uri_string*** '
  ...
  | container |必选项。指导入数据的存储空间名称。<ul><li>导入到本地时，该项名称为数据库用户名。</li><li>导入云存储空间时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。</li></ul><blockquote>**注意**</br>当且仅当存在 scheme 时，才可存在container。</blockquote> |
  |  path  | 必选项。指导入存储空间的资源路径。路径必须以 `/` 开头。 |
  ...
  |  **参数** |**说明**|
  |----------------------------------|--------------------------------------------------------|
  | 数据导入到 S3 | `s3://bucket/path?region={region}&access-key={accessKey}&secret-key={secretKey}` <br> 其中，<ul><li> `s3`：scheme 为 s3。</li><li>`bucket`：container 名称。</li><li>`path`：导入到 S3 存储空间的资源路径。</li><li>`?region={region}&access-key={accessKey}&secret-key={secretKey}`：region、access-key、secret-key 参数值。</li></ul>|
  | 数据导入到本地   | `/home/admin?no-structrue,merge`<br> 其中，<ul><li> `/home/admin`：导入到本地的路径。</li><li>`?no-structrue,merge`：<ul><li> no-structrue：导出时不生成结构。</li><li>merge：合并子文件。</li></ul></li></ul> |
<br>
