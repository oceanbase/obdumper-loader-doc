...
* --storage-uri ' ***storage_uri_string*** '
  ...
  用于标识云存储的统一资源定位符。OBLOADER 4.2.0 及之后的版本支持将数据导入到 OSS 存储空间和 Amazon S3 存储空间。
  ...
  | bucket |指定云存储空间名称。<br>导入数据到云存储空间时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。 |
  |  path  | 指定数据到存储空间的资源路径。路径**必须**以 `/` 开头。 |
  |  parameters  |  指定参数。<br>其中，parameters 可为单个 key 或者 key-value。  |
