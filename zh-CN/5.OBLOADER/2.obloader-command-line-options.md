...
* --storage-uri ' ***storage_uri_string*** '
  ...
  | 数据导入到本地   | `--storage-uri '/home/admin?no-structrue,merge'`<br> 其中，<ul><li> `/home/admin`：导入到本地的路径。</li><li>`?no-structrue,merge`：<ul><li> no-structrue：导出时不生成结构。</li><li>merge：合并子文件。</li></ul></li></ul> |
<br>
