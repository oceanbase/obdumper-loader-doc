...
* --storage-uri ' ***storage_uri_string*** '
  ...
  用于标识云存储的统一资源定位符。OBLOADER 4.2.0 及之后的版本支持将数据导入到 OSS 存储空间和 Amazon S3 存储空间；OBLOADER 4.2.1 及之后的版本支持将数据导入到 Hadoop HDFS 系统。
  ...
  | host |指定云存储空间名称。<ul><li>导入数据到 OSS/S3 时，host 指代 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。</li><li>导入数据到 HDFS 时，该项为 host。</li></ul>  |
  ...
  <ul><li> <code>s3</code>：scheme 为 s3。</li><li><code>host</code>：S3 存储空间的名称。</li><li><code>path</code>：导出到 S3 存储空间的资源路径。</li><li><code>?region={region}&access-key={accessKey}&secret-key={secretKey}</code>：指定 region, access-key, secret-key 参数值。</li></ul>
