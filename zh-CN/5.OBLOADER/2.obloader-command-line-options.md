...
* --storage-uri ' ***storage_uri_string*** '
  ...
  | bucket |必选项。指存储空间名称。<br>导入数据到云存储空间时，该项名称为 bucket，具体请参考 [OSS Bucket](https://help.aliyun.com/document_detail/31827.html#section-yxy-jmt-tdb)。<blockquote>**注意**<ul><li>当且仅当存在 scheme 时，才会同时存在 bucket。</li><li>bucket 必须与 endpoint, access-key, secret-key 参数搭配使用，表示数据位于云存储的指定位置中。</li></ul></blockquote> |
