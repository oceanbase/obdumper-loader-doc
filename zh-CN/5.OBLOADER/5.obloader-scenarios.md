...
**文件类型定义**：CSV 数据文件（后缀名 .csv）请参考 [RFC 4180](http://mirrors.nju.edu.cn/rfc/inline-errata/rfc4180.html) 规范，CSV 数据文件以纯文本形式存储，可通过文本编辑器或者 Excel 等工具直接打开。

**场景描述**：将 `/output` 目录下所有已支持的 CSV 数据文件导入到 Schema USERA 中（OceanBase 4.0.0.0 之前的版本要求提供 sys 租户的密码）。
...
**文件类型定义**：SQL 数据文件（后缀名 .sql）存储的是 Insert SQL 语句，可通过文本编辑器或者 SQL 编辑器等工具直接打开。

**场景描述**：将 `/output` 目录下所有已支持的 SQL 数据文件导入到 Schema USERA 中（OceanBase 4.0.0.0 之前的版本要求提供 sys 租户的密码）。
...
**文件类型定义**：POS 数据文件（默认后缀名 .dat）中的数据是按照固定长度的字节偏移位置所组织的数据，导入时需使用控制文件定义每个字段的固定长度，可通过文本编辑器等工具直接打开。

**场景描述**：将 `/output` 目录下所有已支持的 POS 数据文件导入到 Schema USERA 中（OceanBase 4.0.0.0 之前的版本要求提供 sys 租户的密码）。导入时需要指定控制文件所在的路径，同时使用 `|` 作为列分隔符。
...
**文件类型定义**：CUT 数据文件（后缀名 .dat）中的数据是以单个字符或者字符串分隔的数据文件，可通过文本编辑器等工具打开。

**场景描述**：将 `/output` 目录下所有已支持的 CUT 数据文件导入到 Schema USERA（OceanBase 4.0.0.0 之前的版本要求提供 sys 租户的密码）。导入时需要指定控制文件所在的路径，同时使用 `|@|` 作为列分隔字符串。
