...
* 导出时通过 secure.crt 进行权限认证的方式不可用，目前解决方法：在导数工具根目录下执行命令 `vi bin/obdumper` 后，查找关键字 `-Dproxy.secureFile=file://$PARENT_PATH/conf/secure.crt` 并将其替换为 `-Dproxy.secureFile=$PARENT_PATH/conf/secure.crt`。
